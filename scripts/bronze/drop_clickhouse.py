"""Drop all FotMob bronze tables in ClickHouse."""
...
def parse_args(argv=None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Drop all tables from the bronze ClickHouse schema"
    )
    parser.add_argument(
        "--database",
        default="bronze",
        help="Target database name (default: bronze)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main(argv=None) -> int:
    ...
    try:
        ...
        logger.info("Bronze table discovery completed in %.2f seconds", find_elapsed_seconds)
        ...
        logger.info("Found %s bronze table(s) in %s", total_tables, database)

        for index, table in enumerate(tables, start=1):
            full_table = f"{database}.{table}"
            if args.dry_run:
                logger.info(
                    "[dry-run] Would drop bronze table %s/%s: %s",
                    index,
                    total_tables,
                    full_table,
                )
            ...
            logger.info(
                "Dropped bronze table %s/%s: %s in %.2f seconds",
                index,
                total_tables,
                full_table,
        ...
        if args.dry_run:
            ...
            logger.info("All bronze tables dropped successfully")
        return 0
    finally:
        client.disconnect()
