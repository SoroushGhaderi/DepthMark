---
signal_id: sig_team_shooting_goals_high_quality_only
status: active
entity: team
family: shooting
subfamily: goals
grain: match_team
headline: "High Quality Only"
trigger: "Team records xG per shot > 0.25 (high quality chances) in a finished match (`period = 'All'`)."
row_identity:
  - match_id
  - triggered_side
asset_paths:
  table: gold.sig_team_shooting_goals_high_quality_only
  sql: clickhouse/gold/signal/sig_team_shooting_goals_high_quality_only.sql
  runner: scripts/gold/signal/runners/sig_team_shooting_goals_high_quality_only.py
---
# sig_team_shooting_goals_high_quality_only

## Purpose

Detect team matches where average chance quality per shot is elite (`xG per shot > 0.25`), surfacing compact high-quality attacking profiles rather than volume-heavy shooting alone.

## Tactical And Statistical Logic

- Trigger condition:
  - `triggered_team_xg_per_shot > 0.25`
...
- Similarity gate note: closest active signals are `sig_team_shooting_goals_offensive_masterclass`, `sig_team_shooting_goals_shooting_gallery`, and `sig_team_shooting_goals_xg_overperformance_team`; this signal intentionally coexists as a stricter quality variant (`xG per shot > 0.25`) versus `offensive_masterclass` (`> 0.20`) and still does not require extreme shot volume or extreme goal-vs-xG outperformance.

## Technical Assets

- SQL: `clickhouse/gold/signal/sig_team_shooting_goals_high_quality_only.sql`
- Runner: `scripts/gold/signal/runners/sig_team_shooting_goals_high_quality_only.py`
- Target table: `gold.sig_team_shooting_goals_high_quality_only`

## Example Execution

```bash
python scripts/gold/signal/runners/sig_team_shooting_goals_high_quality_only.py
```
...
| `trigger_threshold_min_xg_per_shot` | Minimum xG-per-shot threshold used by trigger (`0.25`) | Football developer: explicit trigger-rule provenance |
