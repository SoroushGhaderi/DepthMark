---
signal_id: sig_player_possession_passing_perfect_dribbler
status: active
entity: player
family: possession
subfamily: passing
grain: match_player
headline: "Perfect Dribbler"
trigger: "player completes >= 5 successful dribbles with 0 failed dribbles in a single match"
row_identity:
  - match_id
  - triggered_player_id
  - triggered_team_id
asset_paths:
  ...
  sql: clickhouse/gold/dml/signals/player/sig_player_possession_passing_perfect_dribbler.sql
  runner: scripts/gold/run_gold_sql_jobs.py
---
# sig_player_possession_passing_perfect_dribbler

## Purpose

Triggers when a player completes at least 5 successful dribbles with zero failed dribbles in a single match, identifying elite take-on efficiency under match pressure.

## Tactical And Statistical Logic

- Trigger condition:
  - `triggered_player_successful_dribbles >= 5 AND triggered_player_failed_dribbles = 0`
...
```bash
python3 scripts/gold/run_gold_sql_jobs.py --date YYYYMMDD --kind signal --id sig_player_possession_passing_perfect_dribbler
```
...
| `triggered_player_successful_dribbles` | Successful dribbles by triggered player | Football developer: core trigger metric volume guard (`>= 5`) with zero failures |
