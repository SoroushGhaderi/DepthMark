...
trigger: "match is not drawn and the losing team has `long_ball_attempts > 60`"
row_identity:
  - match_id
  - triggered_side
asset_paths:
...
```bash
python3 scripts/gold/run_gold_sql_jobs.py --date YYYYMMDD --kind signal --id sig_team_possession_passing_long_ball_desperation
```
