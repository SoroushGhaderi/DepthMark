...
trigger: "cross_attempts > 40` on full-match stats (`period = 'All'`) for home or away"
row_identity:
  - match_id
  - triggered_side
asset_paths:
  table: gold_signals.sig_team_possession_passing_low_block_frustration
  sql: clickhouse/gold/dml/signals/team/sig_team_possession_passing_low_block_frustration.sql
...
```bash
python3 scripts/gold/run_gold_sql_jobs.py --date YYYYMMDD --kind signal --id sig_team_possession_passing_low_block_frustration
```
