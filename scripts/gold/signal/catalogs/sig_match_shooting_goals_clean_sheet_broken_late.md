...
asset_paths:
  ...
  sql: clickhouse/gold/dml/signals/match/sig_match_shooting_goals_clean_sheet_broken_late.sql
...
```bash
python3 scripts/gold/run_gold_sql_jobs.py --date YYYYMMDD --kind signal --id sig_match_shooting_goals_clean_sheet_broken_late
```
