---
signal_id: sig_player_creativity_playmaking_assist_brace
status: active
entity: player
family: creativity
subfamily: playmaking
grain: match_player
headline: "Assist Brace Playmaker"
trigger: "Player records >= 2 assists in a single finished match."
row_identity:
  - match_id
  - triggered_player_id
  - triggered_team_id
asset_paths:
  table: gold_signals.sig_player_creativity_playmaking_assist_brace
  sql: clickhouse/gold/signal/sig_player_creativity_playmaking_assist_brace.sql
  runner: scripts/gold/signal/runners/sig_player_creativity_playmaking_assist_brace.py
---
# sig_player_creativity_playmaking_assist_brace

## Purpose

Detect player-level playmaking performances where a single player records an assist brace (`>= 2`) in a finished match.

## Tactical And Statistical Logic

- Trigger conditions:
  - `triggered_player_assists >= 2`
  - `match_finished = 1`
...
- Similarity gate note:
  - `sig_player_possession_passing_xa_overperformer`: closest overlap on assist-brace floor, but that signal also requires low xA (`< 0.5`) and is positioned under `possession/passing`.
  - `sig_player_creativity_playmaking_maestro_output`: same creativity/playmaking taxonomy, but key-pass volume (`chances_created >= 5`) rather than realized assist outcomes.
  - Coexistence rationale: this signal is intentionally outcome-centric and taxonomy-aligned for direct playmaking contribution tracking without xA guardrails.

## Technical Assets

- SQL: `clickhouse/gold/signal/sig_player_creativity_playmaking_assist_brace.sql`
- Runner: `scripts/gold/signal/runners/sig_player_creativity_playmaking_assist_brace.py`
- Target table: `gold_signals.sig_player_creativity_playmaking_assist_brace`

## Example Execution

```bash
python scripts/gold/signal/runners/sig_player_creativity_playmaking_assist_brace.py
```
...
| `trigger_threshold_min_assists` | Trigger floor (`2`) | Explicit threshold provenance and QA guard |
| `triggered_player_assists` | Player assists | Core trigger metric |
| `triggered_player_assists_above_threshold` | Assists above trigger floor (`assists - 2`) | Trigger severity beyond activation |
