---
signal_id: sig_match_goalkeeping_defense_box_siege_survival
status: active
entity: team
family: goalkeeping
subfamily: defense
grain: match_team
headline: "Box Siege Survival"
trigger: "One team records >= 40 touches in the opposition box while the opposing team keeps a clean sheet in a finished match (`period = 'All'`)."
row_identity:
  - match_id
  - triggered_side
asset_paths:
  table: gold.sig_match_goalkeeping_defense_box_siege_survival
  sql: clickhouse/gold/signal/sig_match_goalkeeping_defense_box_siege_survival.sql
  runner: scripts/gold/signal/runners/sig_match_goalkeeping_defense_box_siege_survival.py
---
# sig_match_goalkeeping_defense_box_siege_survival

## Purpose

Detects match states where one side survives an opposition box siege (`>= 40` touches in its own
box) while keeping a clean sheet.

## Tactical And Statistical Logic

- Trigger condition:
  - `triggered_team_opposition_box_touches_faced >= 40`
...
- Similarity gate note:
  - `sig_team_goalkeeping_defense_box_evacuation`: closest tactical overlap (clean sheet under extreme box pressure), but this signal is match-scoped with a lower threshold (`>= 40` vs `>= 50`) and placed in the `match_goalkeeping_defense` family.
  - `sig_match_goalkeeping_defense_no_penetration_match`: same entity/family/subfamily and box-touch lens, but that trigger detects very low combined box entries (`< 15`), while this signal captures high-pressure survival with a clean sheet.
  - `sig_match_goalkeeping_defense_goalless_siege_match`: same defensive-survival framing, but driven by 0-0 plus high xG conditions instead of explicit box-touch siege pressure.

## Technical Assets

- SQL: `clickhouse/gold/signal/sig_match_goalkeeping_defense_box_siege_survival.sql`
- Runner: `scripts/gold/signal/runners/sig_match_goalkeeping_defense_box_siege_survival.py`
- Target table: `gold.sig_match_goalkeeping_defense_box_siege_survival`

## Example Execution

```bash
python scripts/gold/signal/runners/sig_match_goalkeeping_defense_box_siege_survival.py
```
...
| `trigger_threshold_min_opposition_box_touches_faced` | Trigger threshold (`40`) | Explicit trigger provenance for reproducibility |
...
| `triggered_team_opposition_box_touches_faced_above_threshold` | Touches faced above threshold (`value - 40`) | Trigger severity beyond activation boundary |
