---
signal_id: sig_match_goalkeeping_defense_coordinated_trap_match
status: active
entity: team
family: goalkeeping
subfamily: defense
grain: match_team
headline: "Coordinated Trap Match"
trigger: "Combined match offsides are at least 12."
row_identity:
  - match_id
  - triggered_side
asset_paths:
  table: gold.sig_match_goalkeeping_defense_coordinated_trap_match
  sql: clickhouse/gold/signal/sig_match_goalkeeping_defense_coordinated_trap_match.sql
  runner: scripts/gold/signal/runners/sig_match_goalkeeping_defense_coordinated_trap_match.py
---
...
- Trigger condition: `(coalesce(offsides_home, 0) + coalesce(offsides_away, 0)) >= 12` from `silver.period_stat` at `period = 'All'`.
...
- Severity is exposed via `match_total_offsides_above_threshold = match_total_offsides - 12`, and side balance is exposed via `match_offside_balance_abs`.
...
- Similarity gate note:
  - `sig_match_goalkeeping_defense_offside_frenzy`: closest overlap (same family/subfamily and offside-only match trigger), while this signal intentionally tightens activation to `>= 12` to represent only the highest offside-intensity matches.
...
- SQL: `clickhouse/gold/signal/sig_match_goalkeeping_defense_coordinated_trap_match.sql`
- Runner: `scripts/gold/signal/runners/sig_match_goalkeeping_defense_coordinated_trap_match.py`
- Target table: `gold.sig_match_goalkeeping_defense_coordinated_trap_match`

## Example Execution

```bash
python scripts/gold/signal/runners/sig_match_goalkeeping_defense_coordinated_trap_match.py
```
...
| `trigger_threshold_min_combined_offsides` | Combined offside threshold baseline (`12`) | Explicit trigger provenance |
...
| `triggered_team_clean_sheet_flag` | 1 when triggered side concedes 0, else 0 | Separates coordinated offside-trap intensity from clean-sheet outcome |
