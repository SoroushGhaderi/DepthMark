---
signal_id: sig_team_goalkeeping_defense_recovery_marathon
status: active
entity: team
family: goalkeeping
subfamily: defense
grain: match_team
headline: "Recovery Marathon"
trigger: "Team records >= 80 total ball recoveries in a single finished match."
row_identity:
  - match_id
  - triggered_side
asset_paths:
  table: gold_signals.sig_team_goalkeeping_defense_recovery_marathon
  sql: clickhouse/gold/dml/signals/team/sig_team_goalkeeping_defense_recovery_marathon.sql
  runner: scripts/gold/run_gold_sql_jobs.py
---
# sig_team_goalkeeping_defense_recovery_marathon

## Purpose

Detect team-level defensive control performances where a side records extreme ball-recovery volume (`>= 80`) and preserve bilateral defending, control, and result context for interpretation.

## Tactical And Statistical Logic

- Trigger condition: `triggered_team_recoveries >= 80` at `match_team` grain for finished matches.
...
- Similarity gate note:
  - `sig_team_goalkeeping_defense_recovery_dominance`: same entity/family/subfamily and core metric, but this signal is a stricter high-tail variant (`>= 80`) versus the broader baseline trigger (`>= 60`).
  ...
  - `sig_player_goalkeeping_defense_recovery_engine`: closest recovery intent, but player-grain trigger (`player >= 12`) versus team-grain aggregate (`team >= 80`).
...
```bash
python3 scripts/gold/run_gold_sql_jobs.py --date YYYYMMDD --kind signal --id sig_team_goalkeeping_defense_recovery_marathon
```
...
| `trigger_threshold_min_recoveries` | Trigger threshold (`80`) | Explicit trigger provenance for reproducibility |
...
| `triggered_team_recoveries_above_threshold` | Recoveries above threshold (`value - 80`) | Trigger severity beyond activation boundary |
