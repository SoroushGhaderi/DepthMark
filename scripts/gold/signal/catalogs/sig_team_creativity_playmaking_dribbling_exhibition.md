---
signal_id: sig_team_creativity_playmaking_dribbling_exhibition
status: active
entity: team
family: creativity
subfamily: playmaking
grain: match_team
headline: "Dribbling Exhibition"
trigger: "Team completes >= 20 successful dribbles."
row_identity:
  - match_id
  - triggered_side
asset_paths:
  table: gold_signals.sig_team_creativity_playmaking_dribbling_exhibition
  sql: clickhouse/gold/dml/signals/team/sig_team_creativity_playmaking_dribbling_exhibition.sql
  runner: scripts/gold/run_gold_sql_jobs.py
---
# sig_team_creativity_playmaking_dribbling_exhibition

## Purpose

Detect team-level dribbling exhibitions where a side completes at least 20 successful dribbles in
a finished match.

## Tactical And Statistical Logic

- Trigger condition:
  - `triggered_team_successful_dribbles >= 20`
- Trigger source:
...
- Similarity gate note:
  - `sig_team_possession_passing_dribble_heavy_attack` overlaps on dribbling profile but triggers
    by dribble attempts (`>= 25`) rather than successful dribble output.
  - `sig_match_possession_passing_dribble_fest` overlaps on successful-dribble context but is
    match-level combined-volume framing, not team-triggered thresholding.
  - `sig_team_creativity_playmaking_crossing_clinic` shares creativity/playmaking taxonomy but uses
    successful crossing trigger logic instead of dribbling.
  - Coexistence rationale: this signal isolates team-level successful dribble exhibition behavior
    as a distinct creativity channel.
...
```bash
python3 scripts/gold/run_gold_sql_jobs.py --date YYYYMMDD --kind signal --id sig_team_creativity_playmaking_dribbling_exhibition
```
...
| `trigger_threshold_min_successful_dribbles` | Trigger floor for successful dribbles (`20`) | Explicit threshold provenance and QA traceability |
...
| `triggered_team_touches_opposition_box` | Triggered-team touches in opposition box | Penetration context behind dribble exhibition |
