---
signal_id: sig_player_possession_passing_xa_overperformer
status: active
entity: player
family: possession
subfamily: passing
grain: match_player
headline: "xA Overperformer"
trigger: "player records 2+ assists with expected assists (xA) < 0.5"
row_identity:
  - match_id
  - triggered_player_id
  - triggered_team_id
asset_paths:
  ...
  sql: clickhouse/gold/dml/signals/player/sig_player_possession_passing_xa_overperformer.sql
  runner: scripts/gold/run_gold_sql_jobs.py
---
# sig_player_possession_passing_xa_overperformer

## Purpose

Triggers when a player records at least 2 assists with less than 0.5 expected assists, identifying high-value creators whose best passes were not converted.

## Tactical And Statistical Logic

- Trigger conditions:
  - `triggered_player_expected_assists < 0.5`
...
```bash
python3 scripts/gold/run_gold_sql_jobs.py --date YYYYMMDD --kind signal --id sig_player_possession_passing_xa_overperformer
```
...
| `home_score` | Home goals at full time | Football developer: outcome context for interpreting assist overperformance vs xA |
| `away_score` | Away goals at full time | Football developer: outcome context for interpreting assist overperformance vs xA |
...
| `triggered_player_expected_assists` | Expected assists generated by triggered player | Football developer: core trigger guard for low expected-assist volume (`< 0.5`) |
| `triggered_player_assists` | Registered assists by triggered player | Football developer: core trigger guard for overperforming assist output (`>= 2`) |
