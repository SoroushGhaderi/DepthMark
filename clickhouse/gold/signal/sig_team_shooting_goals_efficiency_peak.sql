INSERT INTO gold.sig_team_shooting_goals_efficiency_peak (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    trigger_threshold_max_total_shots_exclusive,
    ...
    triggered_team_goals_per_total_shot,
    opponent_goals_per_total_shot,
    goals_per_total_shot_delta,
    ...
    total_shots_below_exclusive_threshold,
...
-- Signal: sig_team_shooting_goals_efficiency_peak
-- Trigger: Team scores >= 4 goals from < 8 total shots in a finished match (`period = 'All'`).
-- Intent: Detect extreme shot-economy finishing where a side produces very high scoring output from minimal shot volume.
...
SELECT
    ...
    toInt32(8) AS trigger_threshold_max_total_shots_exclusive,
    ...
    toFloat32(coalesce(round(
        coalesce(m.home_score, 0) / nullIf(toFloat64(coalesce(ps.total_shots_home, 0)), 0),
        3
    ), 0.0)) AS triggered_team_goals_per_total_shot,
    toFloat32(coalesce(round(
        coalesce(m.away_score, 0) / nullIf(toFloat64(coalesce(ps.total_shots_away, 0)), 0),
        3
    ), 0.0)) AS opponent_goals_per_total_shot,
    toFloat32(round(
        coalesce(round(
            coalesce(m.home_score, 0) / nullIf(toFloat64(coalesce(ps.total_shots_home, 0)), 0),
            3
    ...
    toInt32(greatest(8 - coalesce(ps.total_shots_home, 0), 0)) AS total_shots_below_exclusive_threshold,
...
WHERE m.match_finished = 1
  ...
  AND coalesce(ps.total_shots_home, 0) < 8
...
SELECT
    ...
    toInt32(greatest(8 - coalesce(ps.total_shots_away, 0), 0)) AS total_shots_below_exclusive_threshold,
...
WHERE m.match_finished = 1
  ...
  AND coalesce(ps.total_shots_away, 0) < 8

ORDER BY
    triggered_team_goals_per_total_shot DESC,
