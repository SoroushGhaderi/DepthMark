INSERT INTO gold.sig_match_discipline_cards_foul_parity (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_match_discipline_cards_foul_parity
-- Intent: detect finished matches where both teams commit exactly the same number of fouls.
-- Trigger: fouls_home = fouls_away at period='All'.
WITH base_stats AS (
    SELECT
    ...
    WHERE m.match_finished = 1
      AND m.match_id > 0
      AND coalesce(ps.fouls_home, 0) = coalesce(ps.fouls_away, 0)
)
...
SELECT
    ...
    toInt32(0) AS trigger_threshold_abs_fouls_committed_delta,
    ...
    toInt32(abs(b.fouls_home - b.fouls_away)) AS abs_fouls_committed_delta,
...
SELECT
    ...
    toInt32(abs(b.fouls_away - b.fouls_home)) AS abs_fouls_committed_delta,
    toFloat32(coalesce(round(
        100.0 * b.fouls_away / nullIf(toFloat64(b.match_total_fouls_committed), 0),
        1
    ), 0.0)) AS triggered_team_fouls_share_pct,
...
FROM base_stats AS b
ORDER BY
    match_total_fouls_committed DESC,
    match_total_cards DESC,
