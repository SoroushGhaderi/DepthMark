INSERT INTO gold.sig_match_goalkeeping_defense_shot_block_fest (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_combined_shot_blocks,
    match_combined_shot_blocks,
    match_combined_shot_blocks_above_threshold,
    triggered_team_shot_blocks,
    opponent_shot_blocks,
    shot_blocks_delta,
    triggered_team_shot_block_share_pct,
    opponent_shot_block_share_pct,
    shot_block_share_delta_pct,
...
-- Signal: sig_match_goalkeeping_defense_shot_block_fest
-- Intent: detect matches with extreme combined shot-block output and preserve bilateral defensive
--         resistance context for side-oriented analysis.
-- Trigger: combined shot blocks (home + away) > 15 at period='All'.
WITH base_stats AS (
    SELECT
        m.match_id,
        m.match_date,
        ...
        coalesce(ps.shot_blocks_home, 0) + coalesce(ps.shot_blocks_away, 0)
            AS match_combined_shot_blocks
    ...
    WHERE m.match_finished = 1
      AND m.match_id > 0
      AND (coalesce(ps.shot_blocks_home, 0) + coalesce(ps.shot_blocks_away, 0)) > 15
)
SELECT
    b.match_id,
    b.match_date,
    ...
    toInt32(15) AS trigger_threshold_min_combined_shot_blocks,
    toInt32(b.match_combined_shot_blocks) AS match_combined_shot_blocks,
    toInt32(b.match_combined_shot_blocks - 15) AS match_combined_shot_blocks_above_threshold,
    ...
    toFloat32(round(
        100.0 * b.shot_blocks_home / nullIf(toFloat64(b.match_combined_shot_blocks), 0),
        1
    )) AS triggered_team_shot_block_share_pct,
    toFloat32(round(
        100.0 * b.shot_blocks_away / nullIf(toFloat64(b.match_combined_shot_blocks), 0),
        1
    )) AS opponent_shot_block_share_pct,
    toFloat32(round(
        (
            100.0 * b.shot_blocks_home / nullIf(toFloat64(b.match_combined_shot_blocks), 0)
        ) - (
