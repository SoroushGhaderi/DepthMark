INSERT INTO gold.sig_team_goalkeeping_defense_clearance_barrage (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_team_goalkeeping_defense_clearance_barrage
-- Trigger: team records >= 40 total clearances in a finished match (`period = 'All'`).
-- Intent: detect team-level defensive clearance overloads and preserve bilateral defending,
--         control, and result context for pressure-state interpretation.
...
SELECT
    ...
    toInt32(40) AS trigger_threshold_min_clearances,
    ...
    toInt32(coalesce(ps.clearances_home, 0) - 40) AS triggered_team_clearances_above_threshold,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.clearances_home, 0) >= 40
...
SELECT
    ...
    toInt32(coalesce(ps.clearances_away, 0) - 40) AS triggered_team_clearances_above_threshold,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.clearances_away, 0) >= 40

ORDER BY
    triggered_team_clearances_above_threshold DESC,
    clearances_delta DESC,
    m.match_date DESC,
    m.match_id DESC;
