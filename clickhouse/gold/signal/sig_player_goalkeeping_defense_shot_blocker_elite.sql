...
INSERT INTO gold.sig_player_goalkeeping_defense_shot_blocker_elite (
    ...
    triggered_player_shot_blocks_above_threshold,
...
-- Trigger: defender records at least 4 shot blocks in a finished match.
SELECT
    m.match_id,
    m.match_date,
    m.home_team_id,
    ...
    toInt32(4) AS trigger_threshold_min_shot_blocks,
    toInt32(coalesce(p.blocked_shots, 0)) AS triggered_player_shot_blocks,
    toInt32(coalesce(p.blocked_shots, 0) - 4) AS triggered_player_shot_blocks_above_threshold,
    ...
    toFloat32(coalesce(round(
        ...
        1
    ), 0.0)) AS player_share_of_team_shot_blocks_pct
...
ORDER BY
    triggered_player_shot_blocks DESC,
