INSERT INTO gold.sig_team_creativity_playmaking_unproductive_flair (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_team_creativity_playmaking_unproductive_flair
-- Trigger: team completes >= 25 successful dribbles and scores 0 goals in a finished match.
-- Intent: identify high-flair but scoreless team performances and retain bilateral creative,
...
SELECT
    ...
    toInt32(25) AS trigger_threshold_min_successful_dribbles,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.dribbles_succeeded_home, 0) >= 25
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.dribbles_succeeded_away, 0) >= 25
