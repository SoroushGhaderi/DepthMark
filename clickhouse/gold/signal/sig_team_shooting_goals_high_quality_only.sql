INSERT INTO gold.sig_team_shooting_goals_high_quality_only (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_xg_per_shot,
...
-- Signal: sig_team_shooting_goals_high_quality_only
-- Trigger: Team records xG per shot > 0.25 in a finished match (`period = 'All'`).
-- Intent: Detect team-level high-shot-quality matches where average chance value per attempt is elite.
...
SELECT
    ...
    toFloat32(0.25) AS trigger_threshold_min_xg_per_shot,
...
WHERE m.match_finished = 1
  ...
  AND coalesce(ps.expected_goals_home, 0.0)
        / nullIf(toFloat64(coalesce(ps.total_shots_home, 0)), 0) > 0.25
...
WHERE m.match_finished = 1
  ...
  AND coalesce(ps.expected_goals_away, 0.0)
        / nullIf(toFloat64(coalesce(ps.total_shots_away, 0)), 0) > 0.25

ORDER BY
    triggered_team_xg_per_shot DESC,
