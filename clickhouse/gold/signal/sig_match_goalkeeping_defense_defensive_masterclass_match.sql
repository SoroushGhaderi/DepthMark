INSERT INTO gold.sig_match_goalkeeping_defense_defensive_masterclass_match (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_combined_interceptions,
    match_total_interceptions,
    match_total_interceptions_above_threshold,
    triggered_team_interceptions,
    opponent_interceptions,
    interceptions_delta,
    triggered_team_interceptions_share_pct,
    opponent_interceptions_share_pct,
    interceptions_share_delta_pct,
...
-- Signal: sig_match_goalkeeping_defense_defensive_masterclass_match
-- Intent: detect finished matches with exceptional combined interception volume and preserve
--         bilateral side-oriented defensive, control, and outcome context.
-- Trigger: combined match interceptions (home + away) > 35 at period='All'.
WITH base_stats AS (
    SELECT
        m.match_id,
        m.match_date,
        ...
        coalesce(ps.interceptions_home, 0) + coalesce(ps.interceptions_away, 0)
            AS match_total_interceptions
    ...
    WHERE m.match_finished = 1
      AND m.match_id > 0
      AND coalesce(ps.interceptions_home, 0) + coalesce(ps.interceptions_away, 0) > 35
)
SELECT
    b.match_id,
    b.match_date,
    ...
    toInt32(35) AS trigger_threshold_min_combined_interceptions,
    toInt32(b.match_total_interceptions) AS match_total_interceptions,
    toInt32(b.match_total_interceptions - 35) AS match_total_interceptions_above_threshold,
    ...
    toFloat32(round(
        100.0 * b.interceptions_home / nullIf(toFloat64(b.match_total_interceptions), 0),
        1
    )) AS triggered_team_interceptions_share_pct,
    toFloat32(round(
        100.0 * b.interceptions_away / nullIf(toFloat64(b.match_total_interceptions), 0),
        1
    )) AS opponent_interceptions_share_pct,
    toFloat32(round(
        (
            100.0 * b.interceptions_home / nullIf(toFloat64(b.match_total_interceptions), 0)
        ) - (
        ...
        1
    )) AS interceptions_share_delta_pct,
...
ORDER BY
    match_total_interceptions DESC,
    match_total_interceptions_above_threshold DESC,
    match_date DESC,
    match_id DESC,
    triggered_side;
