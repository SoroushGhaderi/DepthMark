INSERT INTO gold.sig_team_creativity_playmaking_crossing_clinic (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_successful_crosses,
...
-- Signal: sig_team_creativity_playmaking_crossing_clinic
-- Trigger: Team completes >= 10 successful crosses in a single finished match.
-- Intent: identify crossing-led creative clinics driven by high accurate-cross volume,
--         then profile bilateral shot, territory, and chance-quality context.
...
SELECT
    ...
    10 AS trigger_threshold_min_successful_crosses,
    coalesce(ps.accurate_crosses_home, 0) AS triggered_team_successful_crosses,
    coalesce(ps.accurate_crosses_away, 0) AS opponent_successful_crosses,
    coalesce(ps.accurate_crosses_home, 0) - coalesce(ps.accurate_crosses_away, 0)
        AS successful_crosses_delta,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.accurate_crosses_home, 0) >= 10
...
SELECT
    ...
    -- Trigger constants and core crossing values.
    10 AS trigger_threshold_min_successful_crosses,
    coalesce(ps.accurate_crosses_away, 0) AS triggered_team_successful_crosses,
    coalesce(ps.accurate_crosses_home, 0) AS opponent_successful_crosses,
    coalesce(ps.accurate_crosses_away, 0) - coalesce(ps.accurate_crosses_home, 0)
        AS successful_crosses_delta,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.accurate_crosses_away, 0) >= 10

ORDER BY
    assumeNotNull(triggered_team_successful_crosses) DESC,
    assumeNotNull(triggered_team_cross_accuracy_pct) DESC,
    m.match_date DESC,
    m.match_id DESC;
