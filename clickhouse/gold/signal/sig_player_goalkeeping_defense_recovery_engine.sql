...
INSERT INTO gold.sig_player_goalkeeping_defense_recovery_engine (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    triggered_player_recoveries_per_90,
    ...
    player_share_of_team_duels_won_pct
)
-- Signal: sig_player_goalkeeping_defense_recovery_engine
-- Intent: detect defender/midfielder ball-recovery engines and preserve bilateral defensive and control context.
-- Trigger: midfielder or defender records at least 12 recoveries in a finished match.
SELECT
    m.match_id,
    m.match_date,
    m.home_team_id,
    ...
    toInt32(12) AS trigger_threshold_min_recoveries,
    toInt32(coalesce(p.recoveries, 0)) AS triggered_player_recoveries,
    toInt32(coalesce(p.recoveries, 0) - 12) AS triggered_player_recoveries_above_threshold,
    toFloat32(coalesce(round(
        90.0 * coalesce(p.recoveries, 0) / nullIf(toFloat64(coalesce(p.minutes_played, 0)), 0.0),
        1
    ), 0.0)) AS triggered_player_recoveries_per_90,
    ...
    toFloat32(coalesce(round(
        100.0 * coalesce(p.duels_won, 0) / nullIf(toFloat64(multiIf(
            p.team_id = m.home_team_id, coalesce(ps.duels_won_home, 0),
            p.team_id = m.away_team_id, coalesce(ps.duels_won_away, 0),
            0
        )), 0.0),
        1
    ), 0.0)) AS player_share_of_team_duels_won_pct
...
WHERE m.match_finished = 1
  ...
  AND coalesce(pp.usual_playing_position_id, 0) IN (1, 2)
  AND coalesce(p.recoveries, 0) >= 12
ORDER BY
