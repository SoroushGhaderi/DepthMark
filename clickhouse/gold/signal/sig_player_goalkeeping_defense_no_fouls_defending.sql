...
INSERT INTO gold.sig_player_goalkeeping_defense_no_fouls_defending (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    trigger_threshold_min_tackles_duels_won_total,
    trigger_threshold_max_fouls_committed,
    triggered_player_tackles_won,
    triggered_player_duels_won,
    triggered_player_tackles_duels_won_total,
    triggered_player_tackles_duels_won_above_threshold,
    ...
    player_share_of_team_tackles_duels_won_pct
)
-- Signal: sig_player_goalkeeping_defense_no_fouls_defending
-- Intent: identify defender displays of strong duel/tackle output with clean discipline.
-- Trigger: defender records at least 5 combined tackles won + duels won with 0 fouls committed.
SELECT
    m.match_id,
    m.match_date,
    m.home_team_id,
    ...
    toInt32(5) AS trigger_threshold_min_tackles_duels_won_total,
    ...
    toInt32(coalesce(p.tackles_won, 0) + coalesce(p.duels_won, 0))
        AS triggered_player_tackles_duels_won_total,
    toInt32(
        coalesce(p.tackles_won, 0) + coalesce(p.duels_won, 0) - 5
    ) AS triggered_player_tackles_duels_won_above_threshold,
    ...
    toFloat32(coalesce(round(
        100.0 * (coalesce(p.tackles_won, 0) + coalesce(p.duels_won, 0))
            / nullIf(toFloat64(
                multiIf(
                    p.team_id = m.home_team_id, coalesce(ps.tackles_succeeded_home, 0),
                    p.team_id = m.away_team_id, coalesce(ps.tackles_succeeded_away, 0),
        ...
        1
    ), 0.0)) AS player_share_of_team_tackles_duels_won_pct
...
WHERE m.match_finished = 1
  ...
  AND (coalesce(p.tackles_won, 0) + coalesce(p.duels_won, 0)) >= 5
  AND coalesce(p.fouls_committed, 0) = 0
ORDER BY
    triggered_player_tackles_duels_won_total DESC,
