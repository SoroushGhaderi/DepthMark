...
INSERT INTO gold.sig_player_goalkeeping_defense_tackle_master (
    ...
    trigger_threshold_min_tackle_success_pct,
    ...
    triggered_player_tackle_success_above_threshold_pct,
...
-- Trigger: defender wins at least 6 tackles with 100% tackle success in a finished match.
SELECT
    m.match_id,
    m.match_date,
    m.home_team_id,
    ...
    toInt32(6) AS trigger_threshold_min_tackles_won,
    toFloat32(100.0) AS trigger_threshold_min_tackle_success_pct,
    ...
    toInt32(coalesce(p.tackles_won, 0) - 6) AS triggered_player_tackles_won_above_threshold,
    toFloat32(coalesce(
        p.tackle_success_rate,
        round(
            100.0 * coalesce(p.tackles_won, 0)
        ...
        0.0
    ) - 100.0) AS triggered_player_tackle_success_above_threshold_pct,
...
WHERE m.match_finished = 1
  ...
  AND coalesce(p.tackles_won, 0) >= 6
  AND coalesce(p.tackle_attempts, 0) = coalesce(p.tackles_won, 0)
ORDER BY
