INSERT INTO gold.sig_player_goalkeeping_defense_clearance_machine (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_player_goalkeeping_defense_clearance_machine
-- Intent: detect defenders with extreme clearance volume and preserve bilateral defensive-pressure context.
-- Trigger: defender records at least 15 clearances in a finished match.
SELECT
    m.match_id,
    m.match_date,
    m.home_team_id,
    ...
    toInt32(15) AS trigger_threshold_min_clearances,
    toInt32(coalesce(p.clearances, 0)) AS triggered_player_clearances,
    toInt32(coalesce(p.clearances, 0) - 15) AS triggered_player_clearances_above_threshold,
...
WHERE m.match_finished = 1
  ...
  AND coalesce(p.clearances, 0) >= 15
ORDER BY
