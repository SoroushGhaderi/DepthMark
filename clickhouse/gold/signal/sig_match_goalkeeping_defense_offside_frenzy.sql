INSERT INTO gold.sig_match_goalkeeping_defense_offside_frenzy (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_match_goalkeeping_defense_offside_frenzy
...
-- Trigger: combined match offsides (home + away) > 10 at period='All'.
WITH base_stats AS (
    SELECT
        m.match_id,
        m.match_date,
    ...
    WHERE m.match_finished = 1
      AND m.match_id > 0
      AND coalesce(ps.offsides_home, 0) + coalesce(ps.offsides_away, 0) > 10
)
SELECT
    b.match_id,
    b.match_date,
    ...
    toInt32(10) AS trigger_threshold_min_combined_offsides,
    toInt32(b.match_total_offsides) AS match_total_offsides,
    toInt32(b.match_total_offsides - 10) AS match_total_offsides_above_threshold,
