INSERT INTO gold.sig_player_creativity_playmaking_high_value_turnover (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_player_creativity_playmaking_high_value_turnover
-- Trigger: player records >= 5 key passes (chances created) with 0 assists in a finished match.
-- Intent: identify high-output creators whose chance provision was not converted into assists,
--         while preserving bilateral passing and territorial team context.
SELECT
    m.match_id,
    m.match_date,
    m.home_team_id,
    ...
    toInt32(5) AS trigger_threshold_min_key_passes,
    toInt32(0) AS trigger_threshold_max_assists,
    toInt32(coalesce(p.chances_created, 0)) AS triggered_player_key_passes,
    toInt32(coalesce(p.chances_created, 0) - 5) AS triggered_player_key_passes_above_threshold,
    toInt32(coalesce(p.assists, 0)) AS triggered_player_assists,
    toInt32(coalesce(p.chances_created, 0) - coalesce(p.assists, 0))
        AS triggered_player_unconverted_key_passes,
