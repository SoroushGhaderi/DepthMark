...
INSERT INTO gold.sig_player_goalkeeping_defense_sub_defensive_stability (
    ...
    trigger_threshold_max_minutes_played_exclusive,
...
-- Trigger: defensive substitute records at least 3 clearances and at least 2 tackles won in under 15 minutes.
SELECT
    m.match_id,
    m.match_date,
    m.home_team_id,
    ...
    toInt32(coalesce(sp.position_id, 0)) AS triggered_player_position_id,
    toInt32(coalesce(sp.usual_playing_position_id, 0)) AS triggered_player_usual_playing_position_id,
    toInt32(coalesce(sp.substitution_time, 0)) AS triggered_player_substitution_time,

    toInt32(3) AS trigger_threshold_min_clearances,
    toInt32(2) AS trigger_threshold_min_tackles_won,
    toInt32(15) AS trigger_threshold_max_minutes_played_exclusive,

    toInt32(coalesce(p.clearances, 0)) AS triggered_player_clearances,
    toInt32(coalesce(p.clearances, 0) - 3) AS triggered_player_clearances_above_threshold,
    toInt32(coalesce(p.tackles_won, 0)) AS triggered_player_tackles_won,
    toInt32(coalesce(p.tackles_won, 0) - 2) AS triggered_player_tackles_won_above_threshold,
...
INNER JOIN substitute_personnel AS sp
    ON sp.match_id = p.match_id
   AND sp.person_id = p.player_id
...
WHERE m.match_finished = 1
  ...
  AND coalesce(sp.usual_playing_position_id, 0) = 1
  AND coalesce(p.minutes_played, 0) > 0
  AND coalesce(p.minutes_played, 0) < 15
  AND coalesce(p.clearances, 0) >= 3
  AND coalesce(p.tackles_won, 0) >= 2
ORDER BY
