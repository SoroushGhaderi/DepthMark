INSERT INTO gold.sig_player_shooting_goals_big_chance_bottler (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_big_chances_missed,
    triggered_player_big_chances_missed,
    triggered_player_big_chances_missed_above_threshold,
    ...
    triggered_player_big_chances_missed_rate_pct,
    ...
    player_share_of_team_big_chances_missed_pct
)
-- Signal: sig_player_shooting_goals_big_chance_bottler
-- Trigger: player misses >= 3 big chances in a single finished match.
-- Intent: isolate severe single-match wastefulness by player using big-chance-miss
-- volume, with bilateral team and opponent shooting context.

WITH player_big_chance_missed AS (
    SELECT
        s.match_id,
        assumeNotNull(s.player_id) AS triggered_player_id,
        assumeNotNull(s.team_id) AS triggered_team_id,
        countIf(
            coalesce(s.is_goal, 0) = 0
            AND coalesce(s.is_own_goal, 0) = 0
            AND (
                positionCaseInsensitiveUTF8(coalesce(s.situation, ''), 'big chance') > 0
                OR coalesce(s.expected_goals, 0.0) >= 0.30
            )
        ) AS triggered_player_big_chances_missed
    ...
    GROUP BY
        ...
        s.player_id,
        s.team_id
    HAVING triggered_player_big_chances_missed >= 3
)
SELECT
    m.match_id,
    m.match_date,
    ...
    toInt32(3) AS trigger_threshold_min_big_chances_missed,
    toInt32(c.triggered_player_big_chances_missed) AS triggered_player_big_chances_missed,
    toInt32(c.triggered_player_big_chances_missed - 3) AS triggered_player_big_chances_missed_above_threshold,
    ...
    toFloat32(coalesce(
        round(
            100.0 * c.triggered_player_big_chances_missed
            / nullIf(toFloat64(coalesce(p.total_shots, 0)), 0),
            1
        ),
        0.0
    )) AS triggered_player_big_chances_missed_rate_pct,
    ...
    toInt32(multiIf(
        p.team_id = m.home_team_id, coalesce(ps.big_chances_missed_home, 0),
        p.team_id = m.away_team_id, coalesce(ps.big_chances_missed_away, 0),
        0
    )) AS triggered_team_big_chances_missed,
    toInt32(multiIf(
        p.team_id = m.home_team_id, coalesce(ps.big_chances_missed_away, 0),
        p.team_id = m.away_team_id, coalesce(ps.big_chances_missed_home, 0),
        0
    ...
    toFloat32(coalesce(round(
        ...
        1
    ), 0.0)) AS player_share_of_team_big_chances_missed_pct

FROM silver.player_match_stat AS p
INNER JOIN silver.match AS m
    ON m.match_id = p.match_id
INNER JOIN player_big_chance_missed AS c
...
ORDER BY
    triggered_player_big_chances_missed DESC,
    triggered_player_big_chances_missed_above_threshold DESC,
