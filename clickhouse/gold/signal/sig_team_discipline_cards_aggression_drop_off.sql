...
INSERT INTO gold.sig_team_discipline_cards_aggression_drop_off (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    trigger_threshold_max_second_half_fouls,
    triggered_team_fouls_first_half,
    triggered_team_fouls_second_half,
    opponent_fouls_first_half,
    opponent_fouls_second_half,
    triggered_team_fouls_first_half_minus_second_half,
    opponent_fouls_first_half_minus_second_half,
    foul_de_escalation_delta,
    triggered_team_second_half_to_first_half_foul_ratio,
    opponent_second_half_to_first_half_foul_ratio,
    foul_ratio_delta,
    triggered_team_foul_drop_pct,
    opponent_foul_drop_pct,
    foul_drop_delta_pct,
    triggered_team_first_half_fouls_share_pct,
    opponent_first_half_fouls_share_pct,
    first_half_fouls_share_delta_pct,
...
-- Signal: sig_team_discipline_cards_aggression_drop_off
-- Trigger: first-half fouls >= 10 and second-half fouls <= 2.
-- Intent: detect teams that are highly aggressive before halftime but sharply de-escalate after the break.
...
SELECT
    ...
    toInt32(10) AS trigger_threshold_min_first_half_fouls,
    toInt32(2) AS trigger_threshold_max_second_half_fouls,
    ...
    toInt32(coalesce(hfs.home_fouls_first_half, 0) - coalesce(hfs.home_fouls_second_half, 0)) AS triggered_team_fouls_first_half_minus_second_half,
    toInt32(coalesce(hfs.away_fouls_first_half, 0) - coalesce(hfs.away_fouls_second_half, 0)) AS opponent_fouls_first_half_minus_second_half,
    toInt32(
        (coalesce(hfs.home_fouls_first_half, 0) - coalesce(hfs.home_fouls_second_half, 0))
        - (coalesce(hfs.away_fouls_first_half, 0) - coalesce(hfs.away_fouls_second_half, 0))
    ) AS foul_de_escalation_delta,
    ...
    toNullable(toFloat32(round(
        100.0 * (
            coalesce(hfs.home_fouls_first_half, 0) - coalesce(hfs.home_fouls_second_half, 0)
        ) / nullIf(toFloat64(coalesce(hfs.home_fouls_first_half, 0)), 0),
        1
    ))) AS triggered_team_foul_drop_pct,
    toNullable(toFloat32(round(
        100.0 * (
            coalesce(hfs.away_fouls_first_half, 0) - coalesce(hfs.away_fouls_second_half, 0)
        ) / nullIf(toFloat64(coalesce(hfs.away_fouls_first_half, 0)), 0),
        1
    ))) AS opponent_foul_drop_pct,
    toNullable(toFloat32(round(
        (
            100.0 * (
                coalesce(hfs.home_fouls_first_half, 0) - coalesce(hfs.home_fouls_second_half, 0)
        ...
        1
    ))) AS foul_drop_delta_pct,
    toNullable(toFloat32(round(
        100.0 * coalesce(hfs.home_fouls_first_half, 0)
        / nullIf(coalesce(hfs.home_fouls_first_half, 0) + coalesce(hfs.home_fouls_second_half, 0), 0),
        1
    ))) AS triggered_team_first_half_fouls_share_pct,
    toNullable(toFloat32(round(
        100.0 * coalesce(hfs.away_fouls_first_half, 0)
        / nullIf(coalesce(hfs.away_fouls_first_half, 0) + coalesce(hfs.away_fouls_second_half, 0), 0),
        1
    ))) AS opponent_first_half_fouls_share_pct,
    toNullable(toFloat32(round(
        (
            100.0 * coalesce(hfs.home_fouls_first_half, 0)
            / nullIf(coalesce(hfs.home_fouls_first_half, 0) + coalesce(hfs.home_fouls_second_half, 0), 0)
        ...
        1
    ))) AS first_half_fouls_share_delta_pct,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(hfs.home_fouls_first_half, 0) >= 10
  AND coalesce(hfs.home_fouls_second_half, 0) <= 2
...
SELECT
    ...
    toInt32(coalesce(hfs.away_fouls_first_half, 0) - coalesce(hfs.away_fouls_second_half, 0)) AS triggered_team_fouls_first_half_minus_second_half,
    toInt32(coalesce(hfs.home_fouls_first_half, 0) - coalesce(hfs.home_fouls_second_half, 0)) AS opponent_fouls_first_half_minus_second_half,
    toInt32(
        (coalesce(hfs.away_fouls_first_half, 0) - coalesce(hfs.away_fouls_second_half, 0))
        - (coalesce(hfs.home_fouls_first_half, 0) - coalesce(hfs.home_fouls_second_half, 0))
    ) AS foul_de_escalation_delta,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(hfs.away_fouls_first_half, 0) >= 10
  AND coalesce(hfs.away_fouls_second_half, 0) <= 2

ORDER BY
    triggered_team_foul_drop_pct DESC,
    foul_de_escalation_delta DESC,
