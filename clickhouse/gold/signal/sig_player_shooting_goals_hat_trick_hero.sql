INSERT INTO gold.sig_player_shooting_goals_hat_trick_hero (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_player_shooting_goals_hat_trick_hero
-- Trigger: player scores >= 3 goals in the same match.
-- Intent: isolate elite single-match finishing explosions (hat-trick output) with bilateral
-- tactical shooting and possession context.
...
SELECT
    ...
    toInt32(coalesce(p.goals, 0) - 3) AS goals_above_threshold,
