INSERT INTO gold.sig_match_shooting_goals_unproductive_dominance (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_triggered_team_total_shots,
    trigger_threshold_max_triggered_team_big_chances,
    match_total_shots,
    ...
    opponent_also_triggered,
    both_teams_triggered
)
-- Signal: sig_match_shooting_goals_unproductive_dominance
-- Intent: detect finished matches where a side produces very high shot volume but zero big chances,
--         then preserve bilateral shot-quality and control diagnostics for interpretation.
-- Trigger: triggered team has >= 20 total shots and exactly 0 big chances in period='All'.
WITH base_stats AS (
    SELECT
        ...
        toUInt8(
            if(
                coalesce(ps.total_shots_home, 0) >= 20
        ...
        toUInt8(
            if(
                coalesce(ps.total_shots_away, 0) >= 20
    ...
    WHERE m.match_finished = 1
      AND m.match_id > 0
      AND (
            (coalesce(ps.total_shots_home, 0) >= 20 AND coalesce(ps.big_chances_home, 0) = 0)
            OR (coalesce(ps.total_shots_away, 0) >= 20 AND coalesce(ps.big_chances_away, 0) = 0)
      )
)
...
SELECT
    ...
    toInt32(20) AS trigger_threshold_min_triggered_team_total_shots,
    toInt32(0) AS trigger_threshold_max_triggered_team_big_chances,
    b.match_total_shots,
    ...
    b.away_triggered AS opponent_also_triggered,
    toUInt8(if(b.home_triggered = 1 AND b.away_triggered = 1, 1, 0)) AS both_teams_triggered
FROM base_stats AS b
WHERE b.home_triggered = 1
...
SELECT
    ...
    b.home_triggered AS opponent_also_triggered,
    toUInt8(if(b.home_triggered = 1 AND b.away_triggered = 1, 1, 0)) AS both_teams_triggered
FROM base_stats AS b
WHERE b.away_triggered = 1;
