INSERT INTO gold.sig_player_shooting_goals_first_minute_goal (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_first_minute_goals,
    trigger_threshold_max_goal_minute,
    trigger_threshold_max_goal_added_time,
    triggered_player_first_minute_goals,
    triggered_player_first_minute_goal_share_of_match_goals_pct,
    triggered_player_first_minute_first_goal_minute,
    triggered_player_first_minute_first_goal_added_time,
    triggered_player_first_minute_first_goal_effective_minute,
    triggered_player_first_minute_last_goal_minute,
    triggered_player_first_minute_last_goal_added_time,
    triggered_player_first_minute_last_goal_effective_minute,
    ...
    triggered_team_first_minute_non_own_goals,
    opponent_first_minute_non_own_goals,
    first_minute_non_own_goal_delta,
...
-- Signal: sig_player_shooting_goals_first_minute_goal
-- Trigger: player scores >= 1 non-own goal within the first minute (<= 60 seconds proxy).
-- Intent: isolate immediate opening-goal impact by player with bilateral shooting and
-- possession context plus first-minute team goal symmetry.
WITH first_minute_non_own_goal_events AS (
    SELECT
        s.match_id,
    ...
    WHERE s.match_id > 0
      ...
      AND toInt32(coalesce(s.goal_time, s.minute, 0)) <= 1
      AND toInt32(coalesce(s.goal_overload_time, s.minute_added, 0)) = 0
),
player_first_minute_goals AS (
    SELECT
        e.match_id,
        e.team_id,
        e.player_id,
        argMin(e.player_name, tuple(e.goal_effective_minute, e.shot_id)) AS triggered_player_name,
        toInt32(count()) AS triggered_player_first_minute_goals,
        toInt32(argMin(
            e.goal_minute,
            tuple(e.goal_effective_minute, e.shot_id)
        )) AS triggered_player_first_minute_first_goal_minute,
        toInt32(argMin(
            e.goal_added_time,
            tuple(e.goal_effective_minute, e.shot_id)
        )) AS triggered_player_first_minute_first_goal_added_time,
        toInt32(min(e.goal_effective_minute)) AS triggered_player_first_minute_first_goal_effective_minute,
        toInt32(argMax(
            e.goal_minute,
            tuple(e.goal_effective_minute, e.shot_id)
        )) AS triggered_player_first_minute_last_goal_minute,
        toInt32(argMax(
            e.goal_added_time,
            tuple(e.goal_effective_minute, e.shot_id)
        )) AS triggered_player_first_minute_last_goal_added_time,
        toInt32(max(e.goal_effective_minute)) AS triggered_player_first_minute_last_goal_effective_minute
    FROM first_minute_non_own_goal_events AS e
    GROUP BY
        e.match_id,
        e.team_id,
        e.player_id
...
team_first_minute_non_own_goal_totals AS (
    SELECT
        e.match_id,
        e.team_id,
        toInt32(count()) AS team_first_minute_non_own_goals
    FROM first_minute_non_own_goal_events AS e
    GROUP BY
        e.match_id,
        e.team_id
...
SELECT
    ...
    toInt32(1) AS trigger_threshold_min_first_minute_goals,
    toInt32(1) AS trigger_threshold_max_goal_minute,
    toInt32(0) AS trigger_threshold_max_goal_added_time,
    toInt32(pfmg.triggered_player_first_minute_goals) AS triggered_player_first_minute_goals,
    toFloat32(coalesce(round(
        100.0 * pfmg.triggered_player_first_minute_goals
        / nullIf(toFloat64(coalesce(p.goals, 0)), 0),
        1
    ), 0.0)) AS triggered_player_first_minute_goal_share_of_match_goals_pct,
    toInt32(pfmg.triggered_player_first_minute_first_goal_minute)
        AS triggered_player_first_minute_first_goal_minute,
    toInt32(pfmg.triggered_player_first_minute_first_goal_added_time)
        AS triggered_player_first_minute_first_goal_added_time,
    toInt32(pfmg.triggered_player_first_minute_first_goal_effective_minute)
        AS triggered_player_first_minute_first_goal_effective_minute,
    toInt32(pfmg.triggered_player_first_minute_last_goal_minute)
        AS triggered_player_first_minute_last_goal_minute,
    toInt32(pfmg.triggered_player_first_minute_last_goal_added_time)
        AS triggered_player_first_minute_last_goal_added_time,
    toInt32(pfmg.triggered_player_first_minute_last_goal_effective_minute)
        AS triggered_player_first_minute_last_goal_effective_minute,
    toInt32(pfmg.triggered_player_first_minute_goals - 1) AS goals_above_threshold,
    ...
    toInt32(coalesce(triggered_team_first_minute.team_first_minute_non_own_goals, 0))
        AS triggered_team_first_minute_non_own_goals,
    toInt32(coalesce(opponent_first_minute.team_first_minute_non_own_goals, 0))
        AS opponent_first_minute_non_own_goals,
    toInt32(
        coalesce(triggered_team_first_minute.team_first_minute_non_own_goals, 0)
        - coalesce(opponent_first_minute.team_first_minute_non_own_goals, 0)
    ) AS first_minute_non_own_goal_delta,
    toInt32(if(
...
FROM player_first_minute_goals AS pfmg
INNER JOIN silver.player_match_stat AS p
    ON p.match_id = pfmg.match_id
   AND p.player_id = pfmg.player_id
   AND p.team_id = pfmg.team_id
INNER JOIN silver.match AS m
    ON m.match_id = p.match_id
LEFT JOIN silver.period_stat AS ps
    ON ps.match_id = p.match_id
   AND ps.period = 'All'
LEFT JOIN team_first_minute_non_own_goal_totals AS triggered_team_first_minute
    ON triggered_team_first_minute.match_id = pfmg.match_id
   AND triggered_team_first_minute.team_id = pfmg.team_id
LEFT JOIN team_first_minute_non_own_goal_totals AS opponent_first_minute
    ON opponent_first_minute.match_id = pfmg.match_id
   AND opponent_first_minute.team_id = if(
       pfmg.team_id = m.home_team_id, m.away_team_id, m.home_team_id
   )
