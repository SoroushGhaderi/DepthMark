INSERT INTO gold.sig_team_goalkeeping_defense_clean_sheet_efficiency (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    triggered_team_shots_on_target_faced_above_threshold,
...
-- Signal: sig_team_goalkeeping_defense_clean_sheet_efficiency
-- Intent: detect teams that keep a clean sheet despite heavy on-target pressure, preserving
--         bilateral defensive workload, shot-quality-against, and control context.
-- Trigger: team keeps a clean sheet while facing at least 8 shots on target (`period = 'All'`).
...
SELECT
    ...
    toInt32(8) AS trigger_threshold_min_shots_on_target_faced,
    ...
    toInt32(coalesce(ps.shots_on_target_away, 0) - 8)
        AS triggered_team_shots_on_target_faced_above_threshold,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.shots_on_target_away, 0) >= 8
...
SELECT
    ...
    toInt32(coalesce(ps.shots_on_target_home, 0) - coalesce(ps.shots_on_target_away, 0))
        AS shots_on_target_faced_delta,
    toInt32(coalesce(ps.shots_on_target_home, 0) - 8)
        AS triggered_team_shots_on_target_faced_above_threshold,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.shots_on_target_home, 0) >= 8
