INSERT INTO gold.sig_team_goalkeeping_defense_parking_the_bus (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_max_possession_pct,
    trigger_threshold_min_clearances,
    trigger_condition_match_win_required,
    ...
    opponent_clearances,
    clearances_delta,
    triggered_team_clearances_above_threshold,
...
-- Signal: sig_team_goalkeeping_defense_parking_the_bus
-- Intent: detect low-possession winning performances built on heavy clearance volume and compact defensive resistance.
-- Trigger: team wins with less than 30% possession and at least 30 clearances in a finished match.
...
SELECT
    ...
    toFloat32(30.0) AS trigger_threshold_max_possession_pct,
    toInt32(30) AS trigger_threshold_min_clearances,
    toInt8(1) AS trigger_condition_match_win_required,
    ...
    toInt32(coalesce(ps.clearances_home, 0) - 30) AS triggered_team_clearances_above_threshold,
    ...
    toInt32(coalesce(ps.tackles_succeeded_home, 0) - coalesce(ps.tackles_succeeded_away, 0)) AS tackles_won_delta,
    ...
    toFloat32(coalesce(ps.expected_goals_on_target_home, 0)) AS triggered_team_xg_on_target,
    toFloat32(coalesce(ps.expected_goals_on_target_away, 0)) AS opponent_xg_on_target,
    toFloat32(round(
        coalesce(ps.expected_goals_on_target_home, 0) - coalesce(ps.expected_goals_on_target_away, 0),
        3
    )) AS xg_on_target_delta,
...
WHERE m.match_finished = 1
  ...
  AND coalesce(ps.ball_possession_home, 0) < 30
  AND coalesce(ps.clearances_home, 0) >= 30
...
SELECT
    ...
    toInt32(coalesce(ps.clearances_away, 0) - 30) AS triggered_team_clearances_above_threshold,
    ...
    toInt32(coalesce(ps.tackles_succeeded_away, 0) - coalesce(ps.tackles_succeeded_home, 0)) AS tackles_won_delta,
    ...
    toFloat32(coalesce(ps.expected_goals_on_target_away, 0)) AS triggered_team_xg_on_target,
    toFloat32(coalesce(ps.expected_goals_on_target_home, 0)) AS opponent_xg_on_target,
    toFloat32(round(
...
WHERE m.match_finished = 1
  ...
  AND coalesce(ps.ball_possession_away, 0) < 30
  AND coalesce(ps.clearances_away, 0) >= 30
ORDER BY
    triggered_team_clearances DESC,
