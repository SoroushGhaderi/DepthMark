INSERT INTO gold.sig_team_goalkeeping_defense_the_great_wall (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_shot_block_rate_pct,
    trigger_threshold_min_opposition_total_shots,
    ...
    triggered_team_shot_block_rate_above_threshold_pct,
...
-- Signal: sig_team_goalkeeping_defense_the_great_wall
-- Intent: detect teams that block at least half of opposition shot attempts and preserve bilateral
--         defensive workload, control-state, and result context for interpretation.
-- Trigger: blocked shots / opposition total shots >= 50% with opposition total shots >= 1.
...
SELECT
    ...
    toFloat32(50.0) AS trigger_threshold_min_shot_block_rate_pct,
    toInt32(1) AS trigger_threshold_min_opposition_total_shots,
    ...
    toFloat32(coalesce(round(
        100.0 * coalesce(ps.shot_blocks_home, 0)
        / nullIf(toFloat64(coalesce(ps.total_shots_away, 0)), 0.0),
        1
    ), 0.0)) AS triggered_team_shot_block_rate_pct,
    toFloat32(coalesce(round(
        100.0 * coalesce(ps.shot_blocks_away, 0)
        / nullIf(toFloat64(coalesce(ps.total_shots_home, 0)), 0.0),
        1
    ...
    toFloat32(round(
        ...
        1
    )) AS triggered_team_shot_block_rate_above_threshold_pct,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.total_shots_away, 0) >= 1
  AND (
      100.0 * coalesce(ps.shot_blocks_home, 0)
      / nullIf(toFloat64(coalesce(ps.total_shots_away, 0)), 0.0)
  ) >= 50.0
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.total_shots_home, 0) >= 1
  AND (
      100.0 * coalesce(ps.shot_blocks_away, 0)
      / nullIf(toFloat64(coalesce(ps.total_shots_home, 0)), 0.0)
  ) >= 50.0
...
ORDER BY
    triggered_team_shot_block_rate_above_threshold_pct DESC,
    triggered_team_shot_block_rate_pct DESC,
    triggered_team_shot_blocks DESC,
    m.match_date DESC,
    m.match_id DESC;
