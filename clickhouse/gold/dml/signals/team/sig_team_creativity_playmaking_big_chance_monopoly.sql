INSERT INTO gold.sig_team_creativity_playmaking_big_chance_monopoly (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    trigger_threshold_max_opponent_big_chances,
...
-- Signal: sig_team_creativity_playmaking_big_chance_monopoly
-- Trigger: team creates >= 5 big chances while opponent creates 0 in a finished match (`period = 'All'`).
-- Intent: identify team-level creative monopoly matches where one side generates all high-value openings.
...
SELECT
    ...
    toInt32(5) AS trigger_threshold_min_big_chances,
    toInt32(0) AS trigger_threshold_max_opponent_big_chances,
    ...
    toInt32(
        coalesce(ps.big_chances_missed_home, 0) - coalesce(ps.big_chances_missed_away, 0)
    ) AS big_chances_missed_delta,

    toInt32(greatest(
        coalesce(ps.big_chances_home, 0) - coalesce(ps.big_chances_missed_home, 0),
        0
    )) AS triggered_team_big_chances_converted,
    toInt32(greatest(
        coalesce(ps.big_chances_away, 0) - coalesce(ps.big_chances_missed_away, 0),
        0
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.big_chances_home, 0) >= 5
...
SELECT
    ...
    toInt32(
        coalesce(ps.big_chances_missed_away, 0) - coalesce(ps.big_chances_missed_home, 0)
    ) AS big_chances_missed_delta,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.big_chances_away, 0) >= 5
