...
rapid_response_rollup_base AS (
    SELECT
        rrc.match_id AS match_id,
        rrc.triggered_side AS triggered_side,
        toInt32(count()) AS rollup_rapid_response_goals,
        toFloat32(round(avg(toFloat32(rrc.response_gap_minutes)), 2)) AS rollup_average_response_time_minutes,
...
rapid_response_rollup AS (
    SELECT
        ...
        rollup_base.rollup_rapid_response_goals,
        rollup_base.rollup_average_response_time_minutes,
        toInt32(tupleElement(arrayElement(rollup_base.ordered_response_tuples, 1), 5))
            AS triggered_team_first_conceded_goal_minute_before_response,
        toInt32(tupleElement(arrayElement(rollup_base.ordered_response_tuples, 1), 6))
            AS triggered_team_first_conceded_goal_added_time_before_response,
        toInt32(tupleElement(arrayElement(rollup_base.ordered_response_tuples, 1), 4))
            AS triggered_team_first_conceded_goal_effective_minute_before_response,
        toInt32(tupleElement(arrayElement(rollup_base.ordered_response_tuples, 1), 2))
            AS triggered_team_first_response_goal_minute,
        toInt32(tupleElement(arrayElement(rollup_base.ordered_response_tuples, 1), 3))
            AS triggered_team_first_response_goal_added_time,
        toInt32(tupleElement(arrayElement(rollup_base.ordered_response_tuples, 1), 1))
            AS triggered_team_first_response_goal_effective_minute,
        toInt32(tupleElement(arrayElement(rollup_base.ordered_response_tuples, 1), 7))
            AS minutes_to_first_response_goal
    FROM (SELECT * FROM rapid_response_rollup_base) AS rollup_base
)
SELECT
    m.match_id,
    m.match_date,
    ...
    response_rollup.triggered_side,
    if(response_rollup.triggered_side = 'home', m.home_team_id, m.away_team_id) AS triggered_team_id,
    if(response_rollup.triggered_side = 'home', m.home_team_name, m.away_team_name) AS triggered_team_name,
    if(response_rollup.triggered_side = 'home', m.away_team_id, m.home_team_id) AS opponent_team_id,
    if(response_rollup.triggered_side = 'home', m.away_team_name, m.home_team_name) AS opponent_team_name,

    toInt32(2) AS trigger_threshold_max_response_minutes,
    toInt32(1) AS trigger_threshold_min_rapid_response_goals,
    response_rollup.triggered_team_rapid_response_goals,
    toInt32(coalesce(opp_response_rollup.triggered_team_rapid_response_goals, 0)) AS opponent_rapid_response_goals,
    toInt32(
        response_rollup.triggered_team_rapid_response_goals
      - coalesce(opp_response_rollup.triggered_team_rapid_response_goals, 0)
    ) AS rapid_response_goals_delta,
    response_rollup.triggered_team_first_conceded_goal_minute_before_response,
    response_rollup.triggered_team_first_conceded_goal_added_time_before_response,
    response_rollup.triggered_team_first_conceded_goal_effective_minute_before_response,
    response_rollup.triggered_team_first_response_goal_minute,
    response_rollup.triggered_team_first_response_goal_added_time,
    response_rollup.triggered_team_first_response_goal_effective_minute,
    response_rollup.minutes_to_first_response_goal,
    toFloat32(response_rollup.triggered_team_average_response_time_minutes) AS triggered_team_average_response_time_minutes,
    toFloat32(coalesce(opp_response_rollup.triggered_team_average_response_time_minutes, 0.0))
        AS opponent_average_response_time_minutes,
    toFloat32(round(
        response_rollup.triggered_team_average_response_time_minutes
      - coalesce(opp_response_rollup.triggered_team_average_response_time_minutes, 0.0),
        2
    )) AS average_response_time_delta_minutes,
    toInt32(2 - response_rollup.minutes_to_first_response_goal) AS rapid_response_window_margin_minutes,
    toInt32(response_rollup.triggered_team_rapid_response_goals - 1) AS triggered_team_rapid_response_goals_above_threshold,

    toInt32(if(
        response_rollup.triggered_side = 'home',
...
FROM rapid_response_rollup AS response_rollup
INNER JOIN silver.match AS m
    ON m.match_id = response_rollup.match_id
INNER JOIN silver.period_stat AS ps
    ON ps.match_id = m.match_id
   AND ps.match_date = m.match_date
   AND ps.period = 'All'
LEFT JOIN rapid_response_rollup AS opp_response_rollup
    ON opp_response_rollup.match_id = response_rollup.match_id
   AND opp_response_rollup.triggered_side = if(response_rollup.triggered_side = 'home', 'away', 'home')
WHERE m.match_finished = 1
  AND m.match_id > 0

ORDER BY
    triggered_team_rapid_response_goals DESC,
    minutes_to_first_response_goal ASC,
    m.match_date DESC,
    m.match_id DESC;
