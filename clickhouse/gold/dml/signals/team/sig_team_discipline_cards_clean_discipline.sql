INSERT INTO gold.sig_team_discipline_cards_clean_discipline (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_team_discipline_cards_clean_discipline
-- Trigger: team finishes with 0 total cards and <= 7 fouls committed.
-- Intent: detect low-infringement team performances with clean discipline and bilateral context.
...
SELECT
    ...
    toInt32(0) AS trigger_threshold_max_total_cards,
    toInt32(7) AS trigger_threshold_max_fouls_committed,
    ...
    toInt32(7 - coalesce(ps.fouls_home, 0)) AS fouls_committed_below_threshold,
...
WHERE m.match_finished = 1
  ...
  AND coalesce(ps.fouls_home, 0) <= 7
...
SELECT
    ...
    toInt32(7 - coalesce(ps.fouls_away, 0)) AS fouls_committed_below_threshold,
    toFloat32(coalesce(round(
        100.0 * coalesce(ps.fouls_away, 0)
        / nullIf(coalesce(ps.fouls_home, 0) + coalesce(ps.fouls_away, 0), 0),
        1
...
WHERE m.match_finished = 1
  ...
  AND coalesce(ps.fouls_away, 0) <= 7

ORDER BY
    fouls_committed_below_threshold DESC,
    triggered_team_fouls_committed ASC,
    m.match_date DESC,
    m.match_id DESC;
