INSERT INTO gold.sig_team_goalkeeping_defense_tackle_volume_surge (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    tackles_won_delta,
    triggered_team_tackles_won_above_threshold,
...
-- Signal: sig_team_goalkeeping_defense_tackle_volume_surge
-- Intent: detect team-level defensive matches with extreme successful tackle volume.
...
SELECT
    ...
    toInt32(25) AS trigger_threshold_min_tackles_won,
    ...
    toInt32(coalesce(ps.tackles_succeeded_home, 0) - 25) AS triggered_team_tackles_won_above_threshold,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.tackles_succeeded_home, 0) >= 25
...
SELECT
    ...
    toInt32(coalesce(ps.tackles_succeeded_away, 0) - coalesce(ps.tackles_succeeded_home, 0))
        AS tackles_won_delta,
    toInt32(coalesce(ps.tackles_succeeded_away, 0) - 25) AS triggered_team_tackles_won_above_threshold,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.tackles_succeeded_away, 0) >= 25

ORDER BY
    triggered_team_tackles_won_above_threshold DESC,
    tackles_won_delta DESC,
    m.match_date DESC,
    m.match_id DESC;
