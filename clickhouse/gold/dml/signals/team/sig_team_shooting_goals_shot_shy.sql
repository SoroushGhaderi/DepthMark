INSERT INTO gold.sig_team_shooting_goals_shot_shy (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_max_shots_per_half,
    triggered_half_without_shot,
    triggered_team_shots_first_half,
    triggered_team_shots_second_half,
    opponent_shots_first_half,
    opponent_shots_second_half,
    triggered_team_zero_shot_first_half_flag,
    triggered_team_zero_shot_second_half_flag,
    opponent_zero_shot_first_half_flag,
    opponent_zero_shot_second_half_flag,
    half_shot_gap_first_half,
    half_shot_gap_second_half,
...
-- Signal: sig_team_shooting_goals_shot_shy
-- Trigger: Team records 0 total shots in at least one half (`FirstHalf` or `SecondHalf`) in a finished match.
-- Intent: Detect severe half-level attacking droughts and preserve bilateral full-match context for interpretation.
...
SELECT
    ...
    toInt32(0) AS trigger_threshold_max_shots_per_half,
    if(
        coalesce(hss.home_shots_first_half, 0) = 0 AND coalesce(hss.home_shots_second_half, 0) = 0,
        'BothHalves',
        if(coalesce(hss.home_shots_first_half, 0) = 0, 'FirstHalf', 'SecondHalf')
    ) AS triggered_half_without_shot,
    ...
    toInt8(if(coalesce(hss.home_shots_first_half, 0) = 0, 1, 0)) AS triggered_team_zero_shot_first_half_flag,
    toInt8(if(coalesce(hss.home_shots_second_half, 0) = 0, 1, 0)) AS triggered_team_zero_shot_second_half_flag,
    toInt8(if(coalesce(hss.away_shots_first_half, 0) = 0, 1, 0)) AS opponent_zero_shot_first_half_flag,
    toInt8(if(coalesce(hss.away_shots_second_half, 0) = 0, 1, 0)) AS opponent_zero_shot_second_half_flag,

    toInt32(coalesce(hss.home_shots_first_half, 0) - coalesce(hss.away_shots_first_half, 0))
        AS half_shot_gap_first_half,
    toInt32(coalesce(hss.home_shots_second_half, 0) - coalesce(hss.away_shots_second_half, 0))
        AS half_shot_gap_second_half,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND (
      coalesce(hss.home_shots_first_half, 0) = 0
      OR coalesce(hss.home_shots_second_half, 0) = 0
  )
...
SELECT
    ...
    toInt32(0) AS trigger_threshold_max_shots_per_half,
    if(
        coalesce(hss.away_shots_first_half, 0) = 0 AND coalesce(hss.away_shots_second_half, 0) = 0,
        'BothHalves',
        if(coalesce(hss.away_shots_first_half, 0) = 0, 'FirstHalf', 'SecondHalf')
    ) AS triggered_half_without_shot,
    ...
    toInt8(if(coalesce(hss.away_shots_first_half, 0) = 0, 1, 0)) AS triggered_team_zero_shot_first_half_flag,
    toInt8(if(coalesce(hss.away_shots_second_half, 0) = 0, 1, 0)) AS triggered_team_zero_shot_second_half_flag,
    toInt8(if(coalesce(hss.home_shots_first_half, 0) = 0, 1, 0)) AS opponent_zero_shot_first_half_flag,
    toInt8(if(coalesce(hss.home_shots_second_half, 0) = 0, 1, 0)) AS opponent_zero_shot_second_half_flag,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND (
      coalesce(hss.away_shots_first_half, 0) = 0
      OR coalesce(hss.away_shots_second_half, 0) = 0
  );
