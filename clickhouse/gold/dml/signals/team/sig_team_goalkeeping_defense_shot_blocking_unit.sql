INSERT INTO gold.sig_team_goalkeeping_defense_shot_blocking_unit (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_shot_blocks,
...
-- Signal: sig_team_goalkeeping_defense_shot_blocking_unit
-- Intent: detect team-level shot-blocking peaks and preserve bilateral defensive, control,
--         and result context for tactical interpretation.
-- Trigger: team blocks >= 10 shots in a finished match (`period = 'All'`).
...
SELECT
    ...
    toInt32(10) AS trigger_threshold_min_shot_blocks,
    ...
    toInt32(coalesce(ps.shot_blocks_home, 0) - 10) AS triggered_team_shot_blocks_above_threshold,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.shot_blocks_home, 0) >= 10
...
SELECT
    ...
    toInt32(coalesce(ps.shot_blocks_away, 0) - 10) AS triggered_team_shot_blocks_above_threshold,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.shot_blocks_away, 0) >= 10

ORDER BY
    triggered_team_shot_blocks_above_threshold DESC,
    shot_blocks_delta DESC,
    m.match_date DESC,
    m.match_id DESC;
