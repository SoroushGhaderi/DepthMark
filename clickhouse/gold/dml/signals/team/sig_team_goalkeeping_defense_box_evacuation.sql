INSERT INTO gold.sig_team_goalkeeping_defense_box_evacuation (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_team_goalkeeping_defense_box_evacuation
-- Intent: detect clean-sheet defensive escapes where a side absorbs extreme opposition penalty-box pressure.
...
SELECT
    ...
    toInt32(50) AS trigger_threshold_min_opposition_box_touches_faced,
    ...
    toInt32(coalesce(ps.touches_opp_box_away, 0) - 50)
...
SELECT
    ...
    toInt32(coalesce(ps.touches_opp_box_home, 0) - coalesce(ps.touches_opp_box_away, 0))
        AS opposition_box_touches_faced_delta,
    toInt32(coalesce(ps.touches_opp_box_home, 0) - 50)
        AS triggered_team_opposition_box_touches_faced_above_threshold,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.touches_opp_box_home, 0) >= 50
