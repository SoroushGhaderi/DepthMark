INSERT INTO gold.sig_team_goalkeeping_defense_keeper_reliance_index (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_team_goalkeeping_defense_keeper_reliance_index
-- Intent: detect low-possession teams that rely heavily on goalkeeper intervention and preserve
--         bilateral defensive workload, control-state, and outcome context.
-- Trigger: team has >= 5 keeper saves while possession is < 40% in a finished match.
...
SELECT
    ...
    toInt32(5) AS trigger_threshold_min_keeper_saves,
    toFloat32(40.0) AS trigger_threshold_max_possession_pct,
    ...
    toInt32(coalesce(ps.keeper_saves_home, 0) - 5) AS triggered_team_keeper_saves_above_threshold,
    ...
    toFloat32(coalesce(round(
        100.0 * coalesce(ps.keeper_saves_home, 0)
            / nullIf(toFloat64(coalesce(ps.shots_on_target_away, 0)), 0.0),
        1
    ), 0.0)) AS triggered_team_save_rate_pct,
    toFloat32(coalesce(round(
        100.0 * coalesce(ps.keeper_saves_away, 0)
            / nullIf(toFloat64(coalesce(ps.shots_on_target_home, 0)), 0.0),
        1
    ...
    toFloat32(coalesce(round(
        100.0 * coalesce(ps.accurate_passes_home, 0)
            / nullIf(toFloat64(coalesce(ps.pass_attempts_home, 0)), 0.0),
        1
    ), 0.0)) AS triggered_team_pass_accuracy_pct,
    toFloat32(coalesce(round(
        100.0 * coalesce(ps.accurate_passes_away, 0)
            / nullIf(toFloat64(coalesce(ps.pass_attempts_away, 0)), 0.0),
        1
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.keeper_saves_home, 0) >= 5
  AND coalesce(ps.ball_possession_home, 0) < 40.0
...
SELECT
    ...
    toInt32(coalesce(ps.keeper_saves_away, 0) - 5) AS triggered_team_keeper_saves_above_threshold,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.keeper_saves_away, 0) >= 5
  AND coalesce(ps.ball_possession_away, 0) < 40.0
ORDER BY
    triggered_team_keeper_saves DESC,
