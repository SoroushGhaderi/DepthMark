INSERT INTO gold.sig_team_shooting_goals_wasteful_box_presence (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_touches_opposition_box,
    trigger_required_goals,
    triggered_team_touches_opposition_box,
    opponent_touches_opposition_box,
    touches_opposition_box_delta,
    triggered_team_touches_opposition_box_above_threshold,
...
-- Signal: sig_team_shooting_goals_wasteful_box_presence
-- Trigger: Team has >= 30 touches in opposition box but scores 0 goals in a finished match (`period = 'All'`).
-- Intent: Detect territorial penalty-area dominance that fails to convert into goals,
--         while preserving bilateral diagnostics for finishing and chance quality.
...
SELECT
    ...
    toInt32(30) AS trigger_threshold_min_touches_opposition_box,
    ...
    coalesce(ps.touches_opp_box_home, 0) - coalesce(ps.touches_opp_box_away, 0) AS touches_opposition_box_delta,
    coalesce(ps.touches_opp_box_home, 0) - 30 AS triggered_team_touches_opposition_box_above_threshold,
...
SELECT
    ...
    coalesce(ps.touches_opp_box_away, 0) - coalesce(ps.touches_opp_box_home, 0) AS touches_opposition_box_delta,
    coalesce(ps.touches_opp_box_away, 0) - 30 AS triggered_team_touches_opposition_box_above_threshold,
...
ORDER BY
    triggered_team_touches_opposition_box DESC,
