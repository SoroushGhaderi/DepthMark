INSERT INTO gold.sig_player_creativity_playmaking_fullback_playmaker (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    trigger_threshold_min_successful_box_passes_proxy,
    trigger_threshold_required_usual_playing_position_id,
    trigger_fullback_position_ids,
    triggered_player_successful_box_passes_proxy,
    triggered_player_successful_box_passes_proxy_source,
    triggered_player_successful_box_passes_proxy_above_threshold,
    ...
    player_share_of_team_box_passes_proxy_pct,
...
-- Signal: sig_player_creativity_playmaking_fullback_playmaker
-- Trigger: fullback records >= 5 successful passes into the box (proxied by passes_final_third).
-- Intent: identify fullbacks who carry match-level playmaking burden through repeated advanced-area pass progression.
SELECT
    m.match_id,
    m.match_date,
    m.home_team_id,
    ...
    'fullback' AS triggered_player_role_group,
    ...
    toInt32(5) AS trigger_threshold_min_successful_box_passes_proxy,
    toInt32(1) AS trigger_threshold_required_usual_playing_position_id,
    '2,5' AS trigger_fullback_position_ids,
    toInt32(coalesce(p.passes_final_third, 0)) AS triggered_player_successful_box_passes_proxy,
    'passes_final_third_proxy' AS triggered_player_successful_box_passes_proxy_source,
    toInt32(coalesce(p.passes_final_third, 0) - 5)
        AS triggered_player_successful_box_passes_proxy_above_threshold,
    ...
    toFloat32(coalesce(
        ...
        0.0
    )) AS player_share_of_team_box_passes_proxy_pct,
    toFloat32(coalesce(
        round(
...
WHERE m.match_finished = 1
  ...
  AND coalesce(pp.position_id, 0) IN (2, 5)
  AND coalesce(p.passes_final_third, 0) >= 5

ORDER BY
    triggered_player_successful_box_passes_proxy DESC,
