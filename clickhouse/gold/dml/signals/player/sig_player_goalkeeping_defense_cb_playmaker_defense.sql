INSERT INTO gold.sig_player_goalkeeping_defense_cb_playmaker_defense (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    trigger_threshold_min_interceptions,
    trigger_threshold_min_accurate_passes,
    triggered_player_interceptions,
    triggered_player_accurate_passes,
    triggered_player_interceptions_above_threshold,
    triggered_player_accurate_passes_above_threshold,
    ...
    player_share_of_team_interceptions_pct,
...
-- Signal: sig_player_goalkeeping_defense_cb_playmaker_defense
-- Intent: detect center backs who combine elite defensive anticipation with high passing output,
--         then preserve bilateral defensive and control context for interpretation.
...
SELECT
    ...
    toInt32(5) AS trigger_threshold_min_interceptions,
    toInt32(60) AS trigger_threshold_min_accurate_passes,
    ...
    toInt32(coalesce(p.interceptions, 0) - 5) AS triggered_player_interceptions_above_threshold,
    toInt32(coalesce(p.accurate_passes, 0) - 60) AS triggered_player_accurate_passes_above_threshold,
    ...
    toInt32(greatest(coalesce(p.total_passes, 0) - coalesce(p.passes_final_third, 0), 0))
        AS triggered_player_non_final_third_passes_proxy,
    toFloat32(coalesce(round(
        100.0 * greatest(coalesce(p.total_passes, 0) - coalesce(p.passes_final_third, 0), 0)
        / nullIf(toFloat64(coalesce(p.total_passes, 0)), 0.0),
        1
    ), 0.0)) AS triggered_player_non_final_third_pass_share_pct,
    ...
    toFloat32(coalesce(round(
        100.0 * coalesce(p.interceptions, 0) / nullIf(toFloat64(multiIf(
            p.team_id = m.home_team_id, coalesce(ps.interceptions_home, 0),
            p.team_id = m.away_team_id, coalesce(ps.interceptions_away, 0),
            0
        )), 0.0),
        1
    ), 0.0)) AS player_share_of_team_interceptions_pct,
    toFloat32(coalesce(round(
        100.0 * coalesce(p.accurate_passes, 0) / nullIf(toFloat64(multiIf(
            p.team_id = m.home_team_id, coalesce(ps.accurate_passes_home, 0),
            p.team_id = m.away_team_id, coalesce(ps.accurate_passes_away, 0),
            0
        )), 0.0),
        1
    ), 0.0)) AS player_share_of_team_accurate_passes_pct
...
WHERE m.match_finished = 1
  ...
  AND coalesce(p.interceptions, 0) >= 5
  AND coalesce(p.accurate_passes, 0) >= 60
ORDER BY
    triggered_player_interceptions DESC,
