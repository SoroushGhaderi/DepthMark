INSERT INTO gold.sig_player_creativity_playmaking_chance_machine (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_big_chances_created,
    triggered_player_big_chances_created,
    triggered_player_big_chances_created_above_threshold,
    ...
    player_share_of_team_big_chances_pct,
    player_share_of_team_chances_created_pct,
    player_share_of_team_passes_pct
)
-- Signal: sig_player_creativity_playmaking_chance_machine
-- Trigger: player creates >= 3 big chances (Opta definition proxy) in a single finished match.
-- Intent: identify player-level chance-creation engines who repeatedly create high-value opportunities,
--         while preserving bilateral team control, passing, and territorial context.
WITH player_big_chance_creation AS (
    SELECT
        s.match_id,
        ...
        ) AS triggered_player_big_chances_created
    FROM silver.shot AS s
    WHERE s.assist_player_id IS NOT NULL
    GROUP BY
        s.match_id,
        s.assist_player_id
    HAVING triggered_player_big_chances_created >= 3
...
SELECT
    ...
    toInt32(3) AS trigger_threshold_min_big_chances_created,
    toInt32(c.triggered_player_big_chances_created) AS triggered_player_big_chances_created,
    toInt32(c.triggered_player_big_chances_created - 3)
        AS triggered_player_big_chances_created_above_threshold,
    ...
    toFloat32(coalesce(
        round(
            100.0 * c.triggered_player_big_chances_created
            / nullIf(
                toFloat64(multiIf(
                    p.team_id = m.home_team_id, coalesce(ps.big_chances_home, 0),
                    p.team_id = m.away_team_id, coalesce(ps.big_chances_away, 0),
        ...
        0.0
    )) AS player_share_of_team_big_chances_pct,
    toFloat32(coalesce(
        round(
...
INNER JOIN player_big_chance_creation AS c
...
ORDER BY
    triggered_player_big_chances_created DESC,
