INSERT INTO gold.sig_player_goalkeeping_defense_clean_sheet_contributor (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    trigger_threshold_min_total_duel_attempts,
...
-- Signal: sig_player_goalkeeping_defense_clean_sheet_contributor
-- Intent: identify full-match defender clean-sheet performances with perfect duel execution.
...
SELECT
    ...
    toInt32(1) AS trigger_threshold_min_total_duel_attempts,
    toFloat32(100.0) AS trigger_threshold_min_total_duel_success_pct,
...
WHERE m.match_finished = 1
  ...
  AND (coalesce(p.ground_duel_attempts, 0) + coalesce(p.aerial_duel_attempts, 0)) >= 1
  AND (
        coalesce(p.ground_duels_won, 0) + coalesce(p.aerial_duels_won, 0)
      ) = (
        coalesce(p.ground_duel_attempts, 0) + coalesce(p.aerial_duel_attempts, 0)
