INSERT INTO gold.sig_player_goalkeeping_defense_aerial_stronghold (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_aerial_duels_won,
    ...
    triggered_player_duel_win_share_pct,
    ...
    triggered_team_ground_duel_attempts,
    opponent_ground_duel_attempts,
...
-- Signal: sig_player_goalkeeping_defense_aerial_stronghold
-- Intent: detect defenders who dominate aerial contests and preserve bilateral defensive and control context.
-- Trigger: defender wins at least 10 aerial duels in a finished match.
SELECT
    m.match_id,
    m.match_date,
    m.home_team_id,
    ...
    toInt32(10) AS trigger_threshold_min_aerial_duels_won,
    ...
    toFloat32(coalesce(round(
        100.0 * coalesce(p.duels_won, 0)
            / nullIf(coalesce(p.duels_won, 0) + coalesce(p.duels_lost, 0), 0),
        1
    ), 0.0)) AS triggered_player_duel_win_share_pct,
    ...
    toInt32(multiIf(
        p.team_id = m.home_team_id, coalesce(ps.ground_duel_attempts_home, 0),
        p.team_id = m.away_team_id, coalesce(ps.ground_duel_attempts_away, 0),
        0
    )) AS triggered_team_ground_duel_attempts,
    toInt32(multiIf(
        p.team_id = m.home_team_id, coalesce(ps.ground_duel_attempts_away, 0),
        p.team_id = m.away_team_id, coalesce(ps.ground_duel_attempts_home, 0),
        0
    )) AS opponent_ground_duel_attempts,
    toInt32(multiIf(
