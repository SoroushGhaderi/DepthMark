INSERT INTO gold.sig_player_shooting_goals_high_volume_zero_test (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    triggered_team_shots_off_target,
    opponent_shots_off_target,
    ...
    player_share_of_team_shots_off_target_pct
)
-- Signal: sig_player_shooting_goals_high_volume_zero_test
-- Trigger: player records >= 5 total shots, all off-target (wide/over), in a finished match.
-- Intent: isolate high-volume off-frame shooting events where a player repeatedly attempts
-- but never tests the goalkeeper, with bilateral team context for tactical diagnosis.
SELECT
    m.match_id,
    m.match_date,
    m.home_team_id,
    ...
    toInt32(5) AS trigger_threshold_min_total_shots,
    toInt32(0) AS trigger_threshold_max_shots_on_target,
    toInt32(0) AS trigger_threshold_max_blocked_shots,
    ...
    toInt32(coalesce(p.shots_off_target, 0)) AS triggered_player_shots_off_target,
    toInt32(coalesce(p.blocked_shots, 0)) AS triggered_player_blocked_shots,
    toFloat32(coalesce(
        round(
    ...
    toFloat32(coalesce(
        round(
            100.0 * coalesce(p.shots_off_target, 0)
            / nullIf(coalesce(p.total_shots, 0), 0),
            1
        ),
        0.0
    )) AS triggered_player_shots_off_target_pct,
    toFloat32(coalesce(
        round(
    ...
    toInt32(coalesce(p.total_shots, 0) - 5) AS shots_above_threshold,
    toFloat32(round(
        coalesce(p.expected_goals, 0.0) - coalesce(p.goals, 0),
        3
    )) AS expected_goals_wasted,
    ...
    toInt32(multiIf(
        p.team_id = m.home_team_id, coalesce(ps.shots_off_target_home, 0),
        p.team_id = m.away_team_id, coalesce(ps.shots_off_target_away, 0),
        0
    )) AS triggered_team_shots_off_target,
    toInt32(multiIf(
        p.team_id = m.home_team_id, coalesce(ps.shots_off_target_away, 0),
        p.team_id = m.away_team_id, coalesce(ps.shots_off_target_home, 0),
        0
    )) AS opponent_shots_off_target,
    toInt32(multiIf(
        p.team_id = m.home_team_id, coalesce(ps.blocked_shots_home, 0),
        p.team_id = m.away_team_id, coalesce(ps.blocked_shots_away, 0),
        0
    )) AS triggered_team_blocked_shots,
    toInt32(multiIf(
        p.team_id = m.home_team_id, coalesce(ps.blocked_shots_away, 0),
        p.team_id = m.away_team_id, coalesce(ps.blocked_shots_home, 0),
        0
    )) AS opponent_blocked_shots,
    toInt32(multiIf(
    ...
    toFloat32(coalesce(round(
        ...
        1
    ), 0.0)) AS player_share_of_team_shots_off_target_pct
...
WHERE m.match_finished = 1
  ...
  AND coalesce(p.total_shots, 0) >= 5
  AND coalesce(p.shots_on_target, 0) = 0
  AND coalesce(p.blocked_shots, 0) = 0
  AND coalesce(p.shots_off_target, 0) = coalesce(p.total_shots, 0)
