INSERT INTO gold.sig_player_goalkeeping_defense_defensive_workrate_monster (
    ...
    trigger_threshold_min_total_defensive_actions_ticb,
    triggered_player_total_defensive_actions_ticb,
    triggered_player_total_defensive_actions_ticb_above_threshold,
    ...
    triggered_team_total_defensive_actions_ticb,
    opponent_total_defensive_actions_ticb,
    total_defensive_actions_ticb_delta,
...
--         single volume trigger while preserving bilateral team context.
-- Trigger: player records at least 20 total defensive actions (tackles won + interceptions
--          + clearances + shot blocks) in a finished match.
SELECT
    m.match_id,
    m.match_date,
    m.home_team_id,
    ...
    toInt32(20) AS trigger_threshold_min_total_defensive_actions_ticb,
    toInt32(
        coalesce(p.tackles_won, 0)
        + coalesce(p.interceptions, 0)
        + coalesce(p.clearances, 0)
        + coalesce(p.blocked_shots, 0)
    ) AS triggered_player_total_defensive_actions_ticb,
    toInt32(
        coalesce(p.tackles_won, 0)
        + coalesce(p.interceptions, 0)
        + coalesce(p.clearances, 0)
        + coalesce(p.blocked_shots, 0)
        - 20
    ) AS triggered_player_total_defensive_actions_ticb_above_threshold,
    ...
    ) AS triggered_team_total_defensive_actions_ticb,
    toInt32(
        multiIf(
    ...
    ) AS opponent_total_defensive_actions_ticb,
    toInt32(
        (
            multiIf(
                p.team_id = m.home_team_id, coalesce(ps.tackles_succeeded_home, 0),
    ...
    ) AS total_defensive_actions_ticb_delta,
    ...
    toFloat32(coalesce(round(
        ...
        1
    ), 0.0)) AS player_share_of_team_total_defensive_actions_ticb_pct
...
ORDER BY
    triggered_player_total_defensive_actions_ticb DESC,
