INSERT INTO gold.sig_player_goalkeeping_defense_high_line_trapper (
    ...
    triggered_player_offsides_forced_proxy,
    triggered_player_offsides_forced_above_threshold_proxy,
...
-- Trigger: opponent offsides caught by triggered player's team >= 3 in a finished match.
SELECT
    m.match_id,
    m.match_date,
    m.home_team_id,
    ...
    toInt32(3) AS trigger_threshold_min_opponent_offsides_caught,
    toInt32(multiIf(
        p.team_id = m.home_team_id, coalesce(ps.offsides_away, 0),
        p.team_id = m.away_team_id, coalesce(ps.offsides_home, 0),
        0
    )) AS triggered_player_offsides_forced_proxy,
    toInt32(multiIf(
        p.team_id = m.home_team_id, coalesce(ps.offsides_away, 0),
        p.team_id = m.away_team_id, coalesce(ps.offsides_home, 0),
        0
    ) - 3) AS triggered_player_offsides_forced_above_threshold_proxy,
    ...
    toInt32(multiIf(
        ...
        p.team_id = m.away_team_id, coalesce(ps.offsides_home, 0),
        0
    )) AS triggered_team_offsides_caught,
    toInt32(multiIf(
        p.team_id = m.home_team_id, coalesce(ps.offsides_home, 0),
        p.team_id = m.away_team_id, coalesce(ps.offsides_away, 0),
        0
    )) AS opponent_offsides_caught,
    toInt32(multiIf(
        p.team_id = m.home_team_id, coalesce(ps.offsides_away, 0),
        p.team_id = m.away_team_id, coalesce(ps.offsides_home, 0),
        0
        ...
        p.team_id = m.away_team_id, coalesce(ps.offsides_away, 0),
        0
    )) AS offsides_caught_delta,

    toInt32(multiIf(
        p.team_id = m.home_team_id, coalesce(ps.offsides_home, 0),
        p.team_id = m.away_team_id, coalesce(ps.offsides_away, 0),
        0
    )) AS triggered_team_offsides_committed,
    toInt32(multiIf(
        p.team_id = m.home_team_id, coalesce(ps.offsides_away, 0),
        p.team_id = m.away_team_id, coalesce(ps.offsides_home, 0),
        0
    )) AS opponent_offsides_committed,
    toInt32(multiIf(
        p.team_id = m.home_team_id, coalesce(ps.offsides_home, 0),
        p.team_id = m.away_team_id, coalesce(ps.offsides_away, 0),
        0
...
ORDER BY
    triggered_player_offsides_forced_proxy DESC,
