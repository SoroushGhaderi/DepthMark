INSERT INTO gold.sig_player_creativity_playmaking_maestro_output (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_player_creativity_playmaking_maestro_output
-- Trigger: player records >= 5 key passes (chances created) in a single finished match.
-- Intent: identify player-level playmaking maestros with sustained chance creation volume, while
--         preserving bilateral passing and territorial team context.
SELECT
    m.match_id,
    m.match_date,
    m.home_team_id,
