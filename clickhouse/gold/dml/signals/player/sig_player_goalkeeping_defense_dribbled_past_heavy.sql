INSERT INTO gold.sig_player_goalkeeping_defense_dribbled_past_heavy (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    trigger_threshold_min_dribbled_past,
    triggered_player_dribbled_past,
    triggered_player_dribbled_past_above_threshold,
    ...
    triggered_player_duel_loss_share_pct
)
...
-- Signal: sig_player_goalkeeping_defense_dribbled_past_heavy
-- Intent: detect players repeatedly bypassed in duels and preserve bilateral defensive context around that vulnerability.
-- Trigger: player is dribbled past at least 5 times in a finished match.
SELECT
    m.match_id,
    m.match_date,
    m.home_team_id,
    ...
    toInt32(5) AS trigger_threshold_min_dribbled_past,
    toInt32(coalesce(p.dribbled_past, 0)) AS triggered_player_dribbled_past,
    toInt32(coalesce(p.dribbled_past, 0) - 5) AS triggered_player_dribbled_past_above_threshold,
    ...
    toFloat32(coalesce(round(
        100.0 * coalesce(p.duels_lost, 0)
            / nullIf(toFloat64(
                coalesce(p.duels_won, 0) + coalesce(p.duels_lost, 0)
            ), 0.0),
        1
    ), 0.0)) AS triggered_player_duel_loss_share_pct
...
WHERE m.match_finished = 1
  ...
  AND coalesce(p.dribbled_past, 0) >= 5
ORDER BY
    triggered_player_dribbled_past DESC,
    triggered_player_duels_lost DESC,
