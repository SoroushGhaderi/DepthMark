INSERT INTO gold.sig_player_goalkeeping_defense_keeper_save_efficiency (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_shots_on_target_faced,
    trigger_threshold_save_rate_pct,
    ...
    player_share_of_team_shots_on_target_faced_pct,
    player_share_of_team_keeper_saves_pct
)
-- Signal: sig_player_goalkeeping_defense_keeper_save_efficiency
-- Intent: detect goalkeeper matches with perfect shot-stopping efficiency at meaningful on-target volume.
-- Trigger: goalkeeper saves 100% of on-target shots faced with at least 4 shots on target faced.
...
SELECT
    ...
    toInt32(4) AS trigger_threshold_min_shots_on_target_faced,
    toFloat32(100.0) AS trigger_threshold_save_rate_pct,
    ...
    toFloat32(coalesce(round(
        ...
        1
    ), 0.0)) AS triggered_player_save_rate_pct,
    ...
    toFloat32(coalesce(round(
        100.0 * coalesce(kse.triggered_player_shots_on_target_faced, 0)
        / nullIf(
            multiIf(
                p.team_id = m.home_team_id, coalesce(ps.shots_on_target_away, 0),
                p.team_id = m.away_team_id, coalesce(ps.shots_on_target_home, 0),
        ...
        1
    ), 0.0)) AS player_share_of_team_shots_on_target_faced_pct,
    toFloat32(coalesce(round(
        100.0 * coalesce(kse.triggered_player_saves, 0)
        / nullIf(
            multiIf(
        ...
        1
    ), 0.0)) AS player_share_of_team_keeper_saves_pct
...
WHERE m.match_finished = 1
  ...
  AND coalesce(kse.triggered_player_shots_on_target_faced, 0) >= 4
  AND coalesce(kse.triggered_player_goals_conceded, 0) = 0
  AND coalesce(kse.triggered_player_saves, 0) = coalesce(kse.triggered_player_shots_on_target_faced, 0)
ORDER BY
