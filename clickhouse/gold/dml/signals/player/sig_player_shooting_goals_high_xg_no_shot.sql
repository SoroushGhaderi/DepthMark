INSERT INTO gold.sig_player_shooting_goals_high_xg_no_shot (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    player_share_of_team_touches_opposition_box_pct
)
-- Signal: sig_player_shooting_goals_high_xg_no_shot
-- Trigger: player records >= 10 touches in opposition box and 0 total shots.
-- Intent: isolate paradoxical high-presence penalty-area performances where a player
-- reaches dangerous zones repeatedly but never attempts a shot.
...
SELECT
    ...
    toInt32(10) AS trigger_threshold_min_touches_opposition_box,
    ...
    toInt32(coalesce(p.touches, 0)) AS triggered_player_total_touches,
    toInt32(coalesce(p.minutes_played, 0)) AS triggered_player_minutes_played,
    toInt32(coalesce(p.touches_opp_box, 0) - 10) AS box_touches_above_threshold,
    ...
    toFloat32(coalesce(round(
        ...
        1
    ), 0.0)) AS player_share_of_team_touches_opposition_box_pct
