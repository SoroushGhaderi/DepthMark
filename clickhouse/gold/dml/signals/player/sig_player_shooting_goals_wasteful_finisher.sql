INSERT INTO gold.sig_player_shooting_goals_wasteful_finisher (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_player_shooting_goals_wasteful_finisher
-- Trigger: player expected_goals > 1.20 and goals = 0 in the same finished match.
-- Intent: isolate high-chance-volume non-scoring player performances with bilateral
-- team shooting and possession context for wastefulness diagnostics.
...
SELECT
    ...
    toFloat32(1.20) AS trigger_threshold_min_expected_goals,
    toInt32(0) AS trigger_threshold_max_goals,
    ...
    toFloat32(round(coalesce(p.expected_goals, 0.0) - 1.20, 3)) AS expected_goals_above_threshold,
    toFloat32(round(
...
WHERE m.match_finished = 1
  ...
  AND coalesce(p.expected_goals, 0.0) > 1.20
  AND coalesce(p.goals, 0) = 0

ORDER BY
    triggered_player_goal_minus_expected_goals ASC,
