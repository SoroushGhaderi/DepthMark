INSERT INTO gold.sig_player_shooting_goals_late_winner_clutch (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    triggered_player_late_winning_goals,
    triggered_player_first_late_winning_goal_minute,
    triggered_player_first_late_winning_goal_added_time,
    triggered_player_first_late_winning_goal_effective_minute,
    triggered_team_score_before_first_late_winning_goal,
    opponent_score_before_first_late_winning_goal,
    triggered_team_score_after_first_late_winning_goal,
    opponent_score_after_first_late_winning_goal,
    final_goal_margin,
    late_winning_goals_above_threshold,
...
-- Signal: sig_player_shooting_goals_late_winner_clutch
-- Trigger: player scores the decisive non-own winning goal in the 90+ minute in a finished match.
-- Intent: isolate stoppage-time match-winner scorers with score-state evidence and bilateral match context.
WITH late_lead_goal_candidates AS (
    SELECT
        s.match_id,
        ...
        )) AS triggered_team_final_goals,
        toInt32(if(
            coalesce(s.is_home_goal, 0) = 1,
            coalesce(m.away_score, 0),
            coalesce(m.home_score, 0)
        )) AS opponent_final_goals,
        toInt32(if(
            coalesce(s.is_home_goal, 0) = 1,
...
late_winning_goal_events AS (
    SELECT
        ...
        c.final_goal_margin,
        c.shot_id_key
    FROM late_lead_goal_candidates AS c
...
player_late_winning_goals AS (
    SELECT
        ...
        count() AS triggered_player_late_winning_goals,
        toInt32(min(e.goal_minute)) AS triggered_player_first_late_winning_goal_minute,
        toInt32(argMin(
            e.goal_added_time,
            tuple(e.goal_minute, e.goal_added_time, e.shot_id_key)
        )) AS triggered_player_first_late_winning_goal_added_time,
        toInt32(min(e.goal_effective_minute)) AS triggered_player_first_late_winning_goal_effective_minute,
        toInt32(argMin(
            e.triggered_team_score_before,
            tuple(e.goal_minute, e.goal_added_time, e.shot_id_key)
        )) AS triggered_team_score_before_first_late_winning_goal,
        toInt32(argMin(
            e.opponent_score_before,
            tuple(e.goal_minute, e.goal_added_time, e.shot_id_key)
        )) AS opponent_score_before_first_late_winning_goal,
        toInt32(argMin(
            e.triggered_team_score_after,
            tuple(e.goal_minute, e.goal_added_time, e.shot_id_key)
        )) AS triggered_team_score_after_first_late_winning_goal,
        toInt32(argMin(
            e.opponent_score_after,
            tuple(e.goal_minute, e.goal_added_time, e.shot_id_key)
        )) AS opponent_score_after_first_late_winning_goal,
        toInt32(argMin(
            e.final_goal_margin,
            tuple(e.goal_minute, e.goal_added_time, e.shot_id_key)
        )) AS final_goal_margin
    FROM late_winning_goal_events AS e
    GROUP BY
        e.match_id,
        e.team_id,
        e.player_id
...
SELECT
    ...
    toInt32(plw.triggered_player_late_winning_goals) AS triggered_player_late_winning_goals,
    toInt32(plw.triggered_player_first_late_winning_goal_minute) AS triggered_player_first_late_winning_goal_minute,
    toInt32(plw.triggered_player_first_late_winning_goal_added_time) AS triggered_player_first_late_winning_goal_added_time,
    toInt32(plw.triggered_player_first_late_winning_goal_effective_minute) AS triggered_player_first_late_winning_goal_effective_minute,
    toInt32(plw.triggered_team_score_before_first_late_winning_goal) AS triggered_team_score_before_first_late_winning_goal,
    toInt32(plw.opponent_score_before_first_late_winning_goal) AS opponent_score_before_first_late_winning_goal,
    toInt32(plw.triggered_team_score_after_first_late_winning_goal) AS triggered_team_score_after_first_late_winning_goal,
    toInt32(plw.opponent_score_after_first_late_winning_goal) AS opponent_score_after_first_late_winning_goal,
    toInt32(plw.final_goal_margin) AS final_goal_margin,
    toInt32(plw.triggered_player_late_winning_goals - 1) AS late_winning_goals_above_threshold,
...
FROM player_late_winning_goals AS plw
INNER JOIN silver.player_match_stat AS p
    ON p.match_id = plw.match_id
   AND p.team_id = plw.team_id
   AND p.player_id = plw.player_id
INNER JOIN silver.match AS m
    ON m.match_id = plw.match_id
LEFT JOIN silver.period_stat AS ps
    ON ps.match_id = plw.match_id
   AND ps.period = 'All'
...
ORDER BY
    triggered_player_first_late_winning_goal_effective_minute DESC,
    triggered_player_late_winning_goals DESC,
