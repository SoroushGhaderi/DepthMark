INSERT INTO gold.sig_player_creativity_playmaking_assist_brace (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_player_creativity_playmaking_assist_brace
-- Trigger: player records >= 2 assists in a single finished match.
-- Intent: identify high-output single-match playmakers by direct goal contribution via assists,
--         while preserving bilateral team control, chance, and territorial context.
SELECT
    m.match_id,
    m.match_date,
    m.home_team_id,
    ...
    toInt32(2) AS trigger_threshold_min_assists,
    toInt32(coalesce(p.assists, 0)) AS triggered_player_assists,
    toInt32(coalesce(p.assists, 0) - 2) AS triggered_player_assists_above_threshold,
    ...
    toInt32(multiIf(
        p.team_id = m.home_team_id, coalesce(m.away_score, 0),
        p.team_id = m.away_team_id, coalesce(m.home_score, 0),
        0
    )) AS opponent_goals,
    toInt32(multiIf(
