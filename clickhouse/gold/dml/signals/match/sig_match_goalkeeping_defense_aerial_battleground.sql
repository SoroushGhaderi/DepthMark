INSERT INTO gold.sig_match_goalkeeping_defense_aerial_battleground (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_combined_aerials_won,
    match_total_aerials_won,
    match_total_aerials_won_above_threshold,
    match_aerials_won_balance_abs,
    triggered_team_aerials_won,
    opponent_aerials_won,
    aerials_won_delta,
    triggered_team_aerials_won_share_pct,
    opponent_aerials_won_share_pct,
    aerials_won_share_delta_pct,
...
-- Signal: sig_match_goalkeeping_defense_aerial_battleground
-- Intent: detect finished matches with extreme combined aerial-duel volume and preserve
--         bilateral side-oriented defensive workload and control context.
-- Trigger: combined aerial duels won (home + away) > 60 at period='All'.
WITH base_stats AS (
    SELECT
        m.match_id,
        m.match_date,
        ...
        coalesce(ps.aerials_won_home, 0) + coalesce(ps.aerials_won_away, 0) AS match_total_aerials_won
    ...
    WHERE m.match_finished = 1
      AND m.match_id > 0
      AND coalesce(ps.aerials_won_home, 0) + coalesce(ps.aerials_won_away, 0) > 60
)
SELECT
    b.match_id,
    b.match_date,
    ...
    toInt32(60) AS trigger_threshold_min_combined_aerials_won,
    toInt32(b.match_total_aerials_won) AS match_total_aerials_won,
    toInt32(b.match_total_aerials_won - 60) AS match_total_aerials_won_above_threshold,
    toInt32(abs(b.aerials_won_home - b.aerials_won_away)) AS match_aerials_won_balance_abs,
    ...
    toFloat32(round(
        100.0 * b.aerials_won_home / nullIf(toFloat64(b.match_total_aerials_won), 0),
        1
    )) AS triggered_team_aerials_won_share_pct,
    toFloat32(round(
        100.0 * b.aerials_won_away / nullIf(toFloat64(b.match_total_aerials_won), 0),
        1
    )) AS opponent_aerials_won_share_pct,
    toFloat32(round(
        (
            100.0 * b.aerials_won_home / nullIf(toFloat64(b.match_total_aerials_won), 0)
        ) - (
        ...
        1
    )) AS aerials_won_share_delta_pct,
