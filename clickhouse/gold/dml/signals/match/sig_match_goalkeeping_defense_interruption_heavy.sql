INSERT INTO gold.sig_match_goalkeeping_defense_interruption_heavy (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_combined_fouls_and_offsides,
    match_total_fouls_and_offsides,
    match_total_fouls_and_offsides_above_threshold,
    match_total_fouls_committed,
    match_total_offsides,
    match_interruption_balance_abs,
    triggered_team_interruption_events,
    opponent_interruption_events,
    interruption_events_delta,
    triggered_team_interruption_events_share_pct,
    opponent_interruption_events_share_pct,
    interruption_events_share_delta_pct,
...
-- Signal: sig_match_goalkeeping_defense_interruption_heavy
-- Intent: detect finished matches with extreme whistle/offside fragmentation and preserve
--         bilateral side-oriented defensive workload, control state, and outcome context.
-- Trigger: combined fouls and offsides (home + away) > 40 at period='All'.
WITH base_stats AS (
    SELECT
        m.match_id,
        m.match_date,
        ...
        coalesce(ps.fouls_home, 0) + coalesce(ps.fouls_away, 0)
            + coalesce(ps.offsides_home, 0) + coalesce(ps.offsides_away, 0)
            AS match_total_fouls_and_offsides
...
SELECT
    ...
    toInt32(40) AS trigger_threshold_min_combined_fouls_and_offsides,
    toInt32(b.match_total_fouls_and_offsides) AS match_total_fouls_and_offsides,
    toInt32(b.match_total_fouls_and_offsides - 40) AS match_total_fouls_and_offsides_above_threshold,
    ...
    toInt32(abs((b.fouls_home + b.offsides_home) - (b.fouls_away + b.offsides_away)))
        AS match_interruption_balance_abs,
    toInt32(b.fouls_home + b.offsides_home) AS triggered_team_interruption_events,
    toInt32(b.fouls_away + b.offsides_away) AS opponent_interruption_events,
    toInt32((b.fouls_home + b.offsides_home) - (b.fouls_away + b.offsides_away))
        AS interruption_events_delta,
    toFloat32(round(
        100.0 * (b.fouls_home + b.offsides_home)
            / nullIf(toFloat64(b.match_total_fouls_and_offsides), 0),
        1
    )) AS triggered_team_interruption_events_share_pct,
    toFloat32(round(
        100.0 * (b.fouls_away + b.offsides_away)
            / nullIf(toFloat64(b.match_total_fouls_and_offsides), 0),
        1
    )) AS opponent_interruption_events_share_pct,
    toFloat32(round(
        (
            100.0 * (b.fouls_home + b.offsides_home)
                / nullIf(toFloat64(b.match_total_fouls_and_offsides), 0)
        ...
        1
    )) AS interruption_events_share_delta_pct,
...
SELECT
    ...
    toInt32(abs((b.fouls_home + b.offsides_home) - (b.fouls_away + b.offsides_away)))
        AS match_interruption_balance_abs,
    toInt32(b.fouls_away + b.offsides_away) AS triggered_team_interruption_events,
    toInt32(b.fouls_home + b.offsides_home) AS opponent_interruption_events,
    toInt32((b.fouls_away + b.offsides_away) - (b.fouls_home + b.offsides_home))
        AS interruption_events_delta,
    toFloat32(round(
        100.0 * (b.fouls_away + b.offsides_away)
            / nullIf(toFloat64(b.match_total_fouls_and_offsides), 0),
...
ORDER BY
    match_total_fouls_and_offsides DESC,
    match_total_fouls_and_offsides_above_threshold DESC,
    match_date DESC,
    match_id DESC,
    triggered_side;
