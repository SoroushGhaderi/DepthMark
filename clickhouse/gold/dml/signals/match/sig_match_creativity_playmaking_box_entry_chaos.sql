INSERT INTO gold.sig_match_creativity_playmaking_box_entry_chaos (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_match_combined_successful_box_passes_proxy,
    trigger_successful_box_passes_proxy_source,
    match_combined_successful_box_passes_proxy,
    triggered_team_successful_box_passes_proxy,
    opponent_successful_box_passes_proxy,
    successful_box_passes_proxy_delta,
    triggered_team_successful_box_passes_share_pct,
    opponent_successful_box_passes_share_pct,
    successful_box_passes_share_delta_pct,
...
WITH team_box_entry_stats AS (
    SELECT
        p.match_id,
        p.team_id,
        toInt32(sum(coalesce(p.passes_final_third, 0))) AS team_successful_box_passes_proxy,
...
base_stats AS (
    SELECT
        ...
        toInt32(coalesce(hb.team_successful_box_passes_proxy, 0)) AS successful_box_passes_proxy_home,
        toInt32(coalesce(ab.team_successful_box_passes_proxy, 0)) AS successful_box_passes_proxy_away,
        toInt32(
            coalesce(hb.team_successful_box_passes_proxy, 0)
          + coalesce(ab.team_successful_box_passes_proxy, 0)
        ) AS match_combined_successful_box_passes_proxy,

        toInt32(coalesce(hb.team_key_passes, 0)) AS home_key_passes,
        toInt32(coalesce(ab.team_key_passes, 0)) AS away_key_passes,
        toFloat32(coalesce(hb.team_expected_assists, 0.0)) AS home_expected_assists,
        toFloat32(coalesce(ab.team_expected_assists, 0.0)) AS away_expected_assists,
    ...
    INNER JOIN silver.period_stat AS ps
       ...
       AND ps.period = 'All'
    LEFT JOIN team_box_entry_stats AS hb
        ON hb.match_id = m.match_id
       AND hb.team_id = m.home_team_id
    LEFT JOIN team_box_entry_stats AS ab
        ON ab.match_id = m.match_id
       AND ab.team_id = m.away_team_id
...
-- Signal: sig_match_creativity_playmaking_box_entry_chaos
-- Trigger: combined successful passes into the box exceed 50 (proxied by passes_final_third).
-- Intent: identify bilateral box-entry chaos matches where sustained advanced progression volume
--         drives high-tempo creative exchanges.
SELECT
    b.match_id,
    b.match_date,
    b.home_team_id,
    ...
    toInt32(50) AS trigger_threshold_min_match_combined_successful_box_passes_proxy,
    'passes_final_third_proxy' AS trigger_successful_box_passes_proxy_source,
    b.match_combined_successful_box_passes_proxy,

    b.successful_box_passes_proxy_home AS triggered_team_successful_box_passes_proxy,
    b.successful_box_passes_proxy_away AS opponent_successful_box_passes_proxy,
    b.successful_box_passes_proxy_home - b.successful_box_passes_proxy_away AS successful_box_passes_proxy_delta,

    toFloat32(coalesce(round(
        100.0 * b.successful_box_passes_proxy_home
        / nullIf(toFloat64(b.match_combined_successful_box_passes_proxy), 0),
        1
    ), 0.0)) AS triggered_team_successful_box_passes_share_pct,
    toFloat32(coalesce(round(
        100.0 * b.successful_box_passes_proxy_away
        / nullIf(toFloat64(b.match_combined_successful_box_passes_proxy), 0),
        1
    ), 0.0)) AS opponent_successful_box_passes_share_pct,
    toFloat32(round(
        coalesce(round(
            100.0 * b.successful_box_passes_proxy_home
            / nullIf(toFloat64(b.match_combined_successful_box_passes_proxy), 0),
      ...
      - coalesce(round(
        ...
        1
    )) AS successful_box_passes_share_delta_pct,
...
SELECT
    ...
    b.successful_box_passes_proxy_away AS triggered_team_successful_box_passes_proxy,
    b.successful_box_passes_proxy_home AS opponent_successful_box_passes_proxy,
    b.successful_box_passes_proxy_away - b.successful_box_passes_proxy_home AS successful_box_passes_proxy_delta,
