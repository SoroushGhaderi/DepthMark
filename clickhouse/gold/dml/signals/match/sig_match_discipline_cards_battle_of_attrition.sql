INSERT INTO gold.sig_match_discipline_cards_battle_of_attrition (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_match_discipline_cards_battle_of_attrition
-- Intent: Detect high-whistle attrition matches with very high combined foul volume.
-- Trigger: combined fouls (home + away) >= 35 at period='All'.
WITH base_stats AS (
    SELECT
    ...
    WHERE m.match_finished = 1
      AND m.match_id > 0
      AND coalesce(ps.fouls_home, 0) + coalesce(ps.fouls_away, 0) >= 35
)
SELECT
    b.match_id,
    b.match_date,
    ...
    toInt32(35) AS trigger_threshold_min_combined_fouls,
    toInt32(b.match_total_fouls_committed) AS match_total_fouls_committed,
    toInt32(b.match_total_fouls_committed - 35) AS match_total_fouls_above_threshold,
