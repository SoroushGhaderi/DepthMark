INSERT INTO gold.sig_match_goalkeeping_defense_save_fest (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_match_combined_keeper_saves_min,
    match_combined_keeper_saves,
    match_combined_shots_on_target_faced,
    match_combined_goals_conceded,
...
-- Signal: sig_match_goalkeeping_defense_save_fest
-- Intent: detect finished matches with extreme combined save volume and preserve side-oriented
--         defensive workload, resistance quality, and control context.
-- Trigger: combined keeper saves > 12 in period='All'.
WITH base_stats AS (
    SELECT
        ...
        coalesce(m.home_score, 0) + coalesce(m.away_score, 0) AS match_combined_goals_conceded
    ...
    WHERE m.match_finished = 1
      AND m.match_id > 0
      AND (coalesce(ps.keeper_saves_home, 0) + coalesce(ps.keeper_saves_away, 0)) > 12
)
...
SELECT
    ...
    toInt32(13) AS trigger_threshold_match_combined_keeper_saves_min,
    b.match_combined_keeper_saves,
    b.match_combined_shots_on_target_faced,
    b.match_combined_goals_conceded,
...
ORDER BY
    ...
    match_combined_shots_on_target_faced DESC,
    match_date DESC,
    match_id DESC,
    triggered_side;
