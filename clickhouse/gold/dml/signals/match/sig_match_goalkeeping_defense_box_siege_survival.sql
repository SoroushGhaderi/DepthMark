INSERT INTO gold.sig_match_goalkeeping_defense_box_siege_survival (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_opposition_box_touches_faced,
    ...
    triggered_team_opposition_box_touches_faced,
    opponent_opposition_box_touches_faced,
    opposition_box_touches_faced_delta,
    triggered_team_opposition_box_touches_faced_above_threshold,
...
-- Signal: sig_match_goalkeeping_defense_box_siege_survival
-- Intent: detect matches where one side survives an opposition box siege with a clean sheet.
-- Trigger: one team records >= 40 opposition-box touches against the other side, and the other side concedes 0 goals in a finished match (`period = 'All'`).
...
SELECT
    ...
    toInt32(40) AS trigger_threshold_min_opposition_box_touches_faced,
    ...
    toInt32(coalesce(ps.touches_opp_box_away, 0)) AS triggered_team_opposition_box_touches_faced,
    toInt32(coalesce(ps.touches_opp_box_home, 0)) AS opponent_opposition_box_touches_faced,
    toInt32(coalesce(ps.touches_opp_box_away, 0) - coalesce(ps.touches_opp_box_home, 0))
        AS opposition_box_touches_faced_delta,
    toInt32(coalesce(ps.touches_opp_box_away, 0) - 40)
        AS triggered_team_opposition_box_touches_faced_above_threshold,
...
SELECT
    ...
    toInt32(coalesce(ps.touches_opp_box_home, 0)) AS triggered_team_opposition_box_touches_faced,
    toInt32(coalesce(ps.touches_opp_box_away, 0)) AS opponent_opposition_box_touches_faced,
    toInt32(coalesce(ps.touches_opp_box_home, 0) - coalesce(ps.touches_opp_box_away, 0))
        AS opposition_box_touches_faced_delta,
    toInt32(coalesce(ps.touches_opp_box_home, 0) - 40)
        AS triggered_team_opposition_box_touches_faced_above_threshold,
...
ORDER BY
    triggered_team_opposition_box_touches_faced_above_threshold DESC,
