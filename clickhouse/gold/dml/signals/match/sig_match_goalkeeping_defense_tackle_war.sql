INSERT INTO gold.sig_match_goalkeeping_defense_tackle_war (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_combined_tackles_won,
    match_total_tackles_won,
    match_total_tackles_won_above_threshold,
    triggered_team_tackles_won,
    opponent_tackles_won,
    tackles_won_delta,
    triggered_team_tackles_won_share_pct,
    opponent_tackles_won_share_pct,
    tackles_won_share_delta_pct,
...
-- Signal: sig_match_goalkeeping_defense_tackle_war
-- Intent: detect matches where both teams combine for very high successful tackle volume.
-- Trigger: combined successful tackles won (home + away) > 40 at period = 'All'.
WITH base_stats AS (
    SELECT
        m.match_id,
        m.match_date,
    ...
    WHERE m.match_finished = 1
      AND m.match_id > 0
      AND coalesce(ps.tackles_succeeded_home, 0) + coalesce(ps.tackles_succeeded_away, 0) > 40
)
SELECT
    b.match_id,
    b.match_date,
    ...
    toInt32(40) AS trigger_threshold_min_combined_tackles_won,
    toInt32(b.match_total_tackles_won) AS match_total_tackles_won,
    toInt32(b.match_total_tackles_won - 40) AS match_total_tackles_won_above_threshold,
    ...
    toFloat32(round(
        100.0 * b.tackles_won_home / nullIf(toFloat64(b.match_total_tackles_won), 0),
        1
    )) AS triggered_team_tackles_won_share_pct,
    toFloat32(round(
        100.0 * b.tackles_won_away / nullIf(toFloat64(b.match_total_tackles_won), 0),
        1
    )) AS opponent_tackles_won_share_pct,
    toFloat32(round(
        (
            100.0 * b.tackles_won_home / nullIf(toFloat64(b.match_total_tackles_won), 0)
        ) - (
        ...
        1
    )) AS tackles_won_share_delta_pct,
