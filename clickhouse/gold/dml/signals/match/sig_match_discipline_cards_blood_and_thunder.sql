INSERT INTO gold.sig_match_discipline_cards_blood_and_thunder (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_combined_duels_won,
    trigger_threshold_min_combined_fouls,
    match_total_duels_won,
    match_total_duels_won_above_threshold,
    match_total_fouls_committed,
    match_total_fouls_above_threshold,
    triggered_team_duels_won,
    opponent_duels_won,
    duels_won_delta,
    triggered_team_duels_won_share_pct,
    opponent_duels_won_share_pct,
    duels_won_share_delta_pct,
...
-- Signal: sig_match_discipline_cards_blood_and_thunder
-- Intent: Detect physically intense matches with both elevated duel volume and elevated foul pressure.
-- Trigger: combined duels won (home + away) >= 50 AND combined fouls (home + away) >= 25 at period='All'.
WITH base_stats AS (
    SELECT
    ...
    WHERE m.match_finished = 1
      AND m.match_id > 0
      AND coalesce(ps.duels_won_home, 0) + coalesce(ps.duels_won_away, 0) >= 50
      AND coalesce(ps.fouls_home, 0) + coalesce(ps.fouls_away, 0) >= 25
)
SELECT
    b.match_id,
    b.match_date,
    ...
    toInt32(50) AS trigger_threshold_min_combined_duels_won,
    toInt32(25) AS trigger_threshold_min_combined_fouls,
    toInt32(b.match_total_duels_won) AS match_total_duels_won,
    toInt32(b.match_total_duels_won - 50) AS match_total_duels_won_above_threshold,
    toInt32(b.match_total_fouls_committed) AS match_total_fouls_committed,
    toInt32(b.match_total_fouls_committed - 25) AS match_total_fouls_above_threshold,
    ...
    toFloat32(round(
        100.0 * b.duels_won_home / nullIf(toFloat64(b.match_total_duels_won), 0),
        1
    )) AS triggered_team_duels_won_share_pct,
    toFloat32(round(
        100.0 * b.duels_won_away / nullIf(toFloat64(b.match_total_duels_won), 0),
        1
    )) AS opponent_duels_won_share_pct,
    toFloat32(round(
        (
            100.0 * b.duels_won_home / nullIf(toFloat64(b.match_total_duels_won), 0)
        ) - (
