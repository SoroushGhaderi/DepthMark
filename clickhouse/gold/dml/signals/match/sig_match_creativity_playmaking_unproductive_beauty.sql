INSERT INTO gold.sig_match_creativity_playmaking_unproductive_beauty (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_combined_successful_dribbles,
    trigger_threshold_min_combined_key_passes,
    trigger_threshold_match_total_goals,
    match_combined_successful_dribbles,
    match_combined_key_passes,
    match_total_goals,
    ...
    opponent_key_passes,
    key_pass_delta,
    triggered_team_key_pass_share_pct,
    opponent_key_pass_share_pct,
    key_pass_share_delta_pct,
...
base_stats AS (
    SELECT
        ...
        toInt32(coalesce(ps.dribbles_succeeded_home, 0)) AS successful_dribbles_home,
        toInt32(coalesce(ps.dribbles_succeeded_away, 0)) AS successful_dribbles_away,
        toInt32(coalesce(ps.dribble_attempts_home, 0)) AS dribble_attempts_home,
        toInt32(coalesce(ps.dribble_attempts_away, 0)) AS dribble_attempts_away,
        toInt32(
            coalesce(ps.dribbles_succeeded_home, 0) + coalesce(ps.dribbles_succeeded_away, 0)
        ) AS match_combined_successful_dribbles,
        toInt32(coalesce(hc.team_key_passes, 0)) AS home_key_passes,
        toInt32(coalesce(ac.team_key_passes, 0)) AS away_key_passes,
        toInt32(coalesce(hc.team_key_passes, 0) + coalesce(ac.team_key_passes, 0))
            AS match_combined_key_passes,
    ...
    WHERE m.match_finished = 1
      AND m.match_id > 0
      AND (coalesce(ps.dribbles_succeeded_home, 0) + coalesce(ps.dribbles_succeeded_away, 0)) >= 40
      AND (coalesce(hc.team_key_passes, 0) + coalesce(ac.team_key_passes, 0)) >= 20
      AND (coalesce(m.home_score, 0) + coalesce(m.away_score, 0)) = 0
)
-- Signal: sig_match_creativity_playmaking_unproductive_beauty
-- Trigger: Combined successful dribbles >= 40, combined key passes >= 20, and match total goals = 0.
-- Intent: isolate aesthetically rich chance-construction matches that fail to produce goals.
SELECT
    b.match_id,
    b.match_date,
    b.home_team_id,
    ...
    toInt32(40) AS trigger_threshold_min_combined_successful_dribbles,
    toInt32(20) AS trigger_threshold_min_combined_key_passes,
    toInt32(0) AS trigger_threshold_match_total_goals,
    b.match_combined_successful_dribbles,
    b.match_combined_key_passes,
    b.match_total_goals,
    ...
    b.successful_dribbles_home - b.successful_dribbles_away AS successful_dribbles_delta,
    b.dribble_attempts_home AS triggered_team_dribble_attempts,
    b.dribble_attempts_away AS opponent_dribble_attempts,
    b.dribble_attempts_home - b.dribble_attempts_away AS dribble_attempts_delta,
    ...
    toFloat32(round(
        coalesce(round(100.0 * b.successful_dribbles_home / nullIf(toFloat64(b.dribble_attempts_home), 0), 1), 0.0)
        - coalesce(round(100.0 * b.successful_dribbles_away / nullIf(toFloat64(b.dribble_attempts_away), 0), 1), 0.0),
        1
    ...
    toFloat32(coalesce(round(
        100.0 * b.home_key_passes / nullIf(toFloat64(b.match_combined_key_passes), 0),
        1
    ), 0.0)) AS triggered_team_key_pass_share_pct,
    toFloat32(coalesce(round(
        100.0 * b.away_key_passes / nullIf(toFloat64(b.match_combined_key_passes), 0),
        1
    ), 0.0)) AS opponent_key_pass_share_pct,
    toFloat32(round(
        coalesce(round(100.0 * b.home_key_passes / nullIf(toFloat64(b.match_combined_key_passes), 0), 1), 0.0)
        - coalesce(round(100.0 * b.away_key_passes / nullIf(toFloat64(b.match_combined_key_passes), 0), 1), 0.0),
        1
    )) AS key_pass_share_delta_pct,
...
SELECT
    ...
    b.successful_dribbles_away - b.successful_dribbles_home AS successful_dribbles_delta,
    b.dribble_attempts_away AS triggered_team_dribble_attempts,
    b.dribble_attempts_home AS opponent_dribble_attempts,
    b.dribble_attempts_away - b.dribble_attempts_home AS dribble_attempts_delta,
    toFloat32(coalesce(round(
        100.0 * b.successful_dribbles_away / nullIf(toFloat64(b.dribble_attempts_away), 0),
        1
    ), 0.0)) AS triggered_team_dribble_success_pct,
