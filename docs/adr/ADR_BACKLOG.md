...
| `0020-keep-clickhouse-optimization-out-of-setup.md`                  | ClickHouse layer setup is schema-only and excludes full-table optimization SQL.                                                       | Compaction is available only through `scripts/maintenance/optimize_clickhouse.py` with operator-selected scope, not as a setup side effect.                                                |
...
| `0013-fotmob-only-provider-scope.md`                                   | Superseded by ADR 0021 after a concrete Oddspedia source domain was introduced. | Generic provider abstractions remain out of scope; FotMob and Oddspedia have explicit, isolated contracts. |
...
| `0015-redesign-telegram-notification-module.md`                       | Telegram module redesigned: thin transport, Jinja2 templates, dataclasses, single config source, clean break from old modules.    | `src/integrations/telegram/` replaces `metrics_alerts.py`, `alerting.py`, `layer_completion_alerts.py`. Callers must migrate.                                                               |
...
| `0021-add-oddspedia-as-source-domain.md`                               | Oddspedia is an isolated source domain; FotMob remains the canonical fixture reference and matching is a Silver conformance output. | Oddspedia raw facts remain outside `bronze.*`; ambiguous cross-source matches are never auto-selected. |
...
| P1       | FotMob-only provider scope and abstraction boundary               | Superseded by `0021-add-oddspedia-as-source-domain.md`                                                     | Oddspedia is now a concrete, isolated second source domain. Generic provider abstractions remain intentionally out of scope.                                                                                                                                                | ADR 0021 defines the explicit source-domain boundary and audited resolution contract.                                                              |
| P1       | `src/` workflow boundary behind script entry points               | Accepted in `0010-introduce-src-application-services-behind-script-entrypoints.md`                         | ADR 0002 keeps CLI subprocesses as the orchestration boundary, while source-specific and warehouse modules under `src/` carry reusable behavior. Their placement changes testing, imports, and failure handling without changing the command surface.                                                        | ADR deciding whether scripts remain the application boundary or whether stable source and warehouse APIs should be introduced behind the same CLI entry points. |
...
| P3       | Bronze filesystem retention and DLQ replay policy                 | Accepted in `0012-bronze-filesystem-retention-and-dlq-replay-policy.md`                                    | Bronze is the only filesystem-backed layer, and DLQ fallback exists under `src/fotmob/bronze/dead_letter.py`, but retention and replay contracts are not documented as an architectural decision.                                                                                                           | ADR defining local/raw payload retention, DLQ replay ownership, and safe cleanup constraints.                                                      |
...
| ------- | ----------------------------------- | -------------------------------------------------------------------- |
| 1       | FotMob-only provider scope          | Superseded by `0021-add-oddspedia-as-source-domain.md`               |
...
| 8       | Isolated Oddspedia source domain | Accepted in `0021-add-oddspedia-as-source-domain.md`; supersedes FotMob-only scope |
